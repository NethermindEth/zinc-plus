...
//! with i128 codeword coefficients, num_rows=2, and the 32x full-folded pipeline
//! (BinaryPoly<32> → 16 → 8 → 4 → 2 → 1).
//!
//! Displays proof size (raw and DEFLATE-compressed).
//!
//! Run with:
//!   cargo bench --bench steps_chacha20_full_folded -p zinc-snark --features "parallel simd asm qx-constraints true-ideal"
...
struct ChaCha20FullFoldZipTypes<CwCoeff, const D_PLUS_ONE: usize>(PhantomData<CwCoeff>);

impl<CwCoeff, const D_PLUS_ONE: usize> ZipTypes for ChaCha20FullFoldZipTypes<CwCoeff, D_PLUS_ONE>
where
...
/// 32x Full-folded PCS types — BinaryPoly<1> codewords with i128 coefficients.
/// Uses PnttConfigF2_16R4B32<3> (BASE_LEN=32, DEPTH=3):
///   INPUT_LEN = 32 × 512 = 16384,
///   OUTPUT_LEN = 128 × 512 = 65536 (rate 1/4).
/// With num_rows=2: 2 × 16384 = 32768 = 2^15, supporting num_vars=15.
type FoldedZt = ChaCha20FullFoldZipTypes<i128, 1>;
...
fn chacha20_full_folded_stepwise(c: &mut Criterion) {
    use zinc_chacha20_uair::CyclotomicIdeal;

    let mem_tracker = MemoryTracker::start();

    let mut group = c.benchmark_group(format!(
        "ChaCha20 32x-FullFolded Hybrid GKR c=2 Steps (grind={})",
        <FoldedZt as ZipTypes>::GRINDING_BITS,
    ));
    group.sample_size(30);

    // -- Full-folded PCS params -------------------------------------------
    let folded_extra_vars: usize = 5; // Five splits: 32→16→8→4→2→1
    let folded_num_vars = CHACHA_NUM_VARS + folded_extra_vars;
    let pcs_num_rows: usize = 2;
    ...
    eprintln!(
        "\n── ChaCha20 32x Full-Folded Trace Info ─────────────────",
    );
    eprintln!(
    ...
    // ── 2. E2E Total Prover ─────────────────────────────────────────
    // Warmup run.
    let _warmup_proof = zinc_snark::pipeline::prove_hybrid_gkr_logup_full_folded::<
        ChaCha20UairBp, ChaCha20UairQx, FoldedZt, FoldedLc, 32, 16, 8, 4, 2, 1, UNCHECKED,
    >(
    ...
    // Print prover pipeline timing breakdown.
    {
        ...
        eprintln!("\n── ChaCha20 32x Full-Folded Hybrid GKR c=2 Prover Pipeline Timing ──");
    ...
    // ── Proof size ───────────────────────────────────────────────────
    {
        ...
        eprintln!("\n=== ChaCha20 32x Full-Folded Hybrid GKR c=2 Proof Size ===");
    ...
    group.bench_function("E2E/Prover (32x Full-Folded Hybrid GKR c=2)", |b| {
        b.iter_custom(|iters| {
    ...
    // ── 3. E2E Total Verifier ───────────────────────────────────────
    {
        ...
        let r = zinc_snark::pipeline::verify_classic_logup_full_folded::<
            ChaCha20UairBp, ChaCha20UairQx, FoldedZt, FoldedLc, 32, 1, UNCHECKED, _, _, _, _,
        >(
        ...
        let t = &r.timing;
        eprintln!("\n── ChaCha20 32x Full-Folded Verifier step timing ────────");
    ...
    group.bench_function("E2E/Verifier (32x Full-Folded Hybrid GKR c=2)", |b| {
        b.iter(|| {
            #[cfg(not(feature = "true-ideal"))]
            fn qx_ideal_closure(_: &IdealOrZero<ChaCha20QxIdeal>) -> zinc_snark::pipeline::TrivialIdeal {
                zinc_snark::pipeline::TrivialIdeal
...
criterion_group!(benches, chacha20_full_folded_stepwise);
