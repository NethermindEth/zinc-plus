//! ChaCha20 end-to-end proving and verification benchmark for **1 KB messages**
//! using the 4x-folded pipeline.
//!
//! Proves **2048 chained ChaCha20 quarter-rounds** (num_vars=11, 2048 rows)
//! representing 25.6 ChaCha20 blocks (~1638 bytes of keystream), sufficient
//! to encrypt a 1 KB (1024-byte) message.
//!
//! Uses `PnttConfigF2_16R4B4<3>` (BASE_LEN=4, DEPTH=3, INPUT_LEN=2048)
//! with num_rows=4 and the 4x-folded pipeline (BinaryPoly<32> -> 16 -> 8).
//!
//! Displays proof size (raw and DEFLATE-compressed).
//!
//! Run with:
//!   cargo bench --bench steps_chacha20_1kb_folded -p zinc-snark --features "parallel simd asm qx-constraints true-ideal"
...
struct ChaCha20_1kbZipTypes<CwCoeff, const D_PLUS_ONE: usize>(PhantomData<CwCoeff>);

impl<CwCoeff, const D_PLUS_ONE: usize> ZipTypes for ChaCha20_1kbZipTypes<CwCoeff, D_PLUS_ONE>
where
...
/// PCS types for ChaCha20 1 KB benchmark: BinaryPoly<8> codewords.
/// Uses PnttConfigF2_16R4B4<3> (BASE_LEN=4, DEPTH=3):
///   INPUT_LEN = 4 × 2^9 = 2048,
///   OUTPUT_LEN = 16 × 512 = 8192 (rate 1/4).
/// With num_rows=4: 4 × 2048 = 8192 = 2^13, supporting num_vars=11.
type FoldedZt = ChaCha20_1kbZipTypes<i64, 8>;
...
/// 2^11 = 2048 rows = 2048 chained quarter-rounds.
/// ≈ 25.6 ChaCha20 blocks ≈ 1638 bytes of keystream (covers 1 KB messages).
const CHACHA_NUM_VARS: usize = 11;
...
fn chacha20_1kb_folded_stepwise(c: &mut Criterion) {
    use zinc_chacha20_uair::CyclotomicIdeal;

    let mem_tracker = MemoryTracker::start();

    let mut group = c.benchmark_group(format!(
        "ChaCha20-1KB 4x-Folded Hybrid GKR c=2 Steps (grind={})",
    ...
    // Print prover pipeline timing breakdown.
    {
        ...
        eprintln!("\n── ChaCha20-1KB 4x-Folded Hybrid GKR c=2 Prover Pipeline Timing ──");
    ...
    // ── Proof size (raw + compressed) ───────────────────────────────
    {
        ...
        eprintln!("\n=== ChaCha20-1KB 4x-Folded Hybrid GKR c=2 Proof Size ===");
    ...
    // ── 3. E2E Total Verifier ───────────────────────────────────────
    {
        ...
        let t = &r.timing;
        eprintln!("\n── ChaCha20-1KB Verifier step timing ────────────────────");
...
criterion_group!(benches, chacha20_1kb_folded_stepwise);
