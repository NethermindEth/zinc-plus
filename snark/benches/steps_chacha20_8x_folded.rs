...
//!
//! Uses `PnttConfigF2_16R4B16<3>` (BASE_LEN=16, DEPTH=3, INPUT_LEN=8192)
//! with i128 codeword coefficients, num_rows=1, and the 8x-folded pipeline
//! (BinaryPoly<32> → 16 → 8 → 4).
//!
//! Displays proof size (raw and DEFLATE-compressed).
//!
//! Run with:
//!   cargo bench --bench steps_chacha20_8x_folded -p zinc-snark --features "parallel simd asm qx-constraints true-ideal"
...
struct ChaCha20_8xFoldZipTypes<CwCoeff, const D_PLUS_ONE: usize>(PhantomData<CwCoeff>);

impl<CwCoeff, const D_PLUS_ONE: usize> ZipTypes for ChaCha20_8xFoldZipTypes<CwCoeff, D_PLUS_ONE>
where
...
/// 8x-folded PCS types — BinaryPoly<4> codewords with i128 coefficients.
/// Uses PnttConfigF2_16R4B16<3> (BASE_LEN=16, DEPTH=3):
///   INPUT_LEN = 16 × 512 = 8192,
///   OUTPUT_LEN = 64 × 512 = 32768 (rate 1/4).
/// With num_rows=1: 1 × 8192 = 8192 = 2^13, supporting num_vars=13.
type FoldedZt = ChaCha20_8xFoldZipTypes<i128, 4>;
type FoldedLc = IprsCode<
    FoldedZt,
    PnttConfigF2_16R4B16<3>,
...
fn chacha20_8x_folded_stepwise(c: &mut Criterion) {
    use zinc_chacha20_uair::CyclotomicIdeal;

    let mem_tracker = MemoryTracker::start();

    let mut group = c.benchmark_group(format!(
        "ChaCha20 8x-Folded Hybrid GKR c=2 Steps (grind={})",
        <FoldedZt as ZipTypes>::GRINDING_BITS,
    ));
    group.sample_size(30);

    // -- 8x-folded PCS params -------------------------------------------
    let folded_extra_vars: usize = 3; // Three splits: 32→16→8→4
    ...
    // ── 2. E2E Total Prover ─────────────────────────────────────────
    // Warmup run.
    let _warmup_proof = zinc_snark::pipeline::prove_hybrid_gkr_logup_8x_folded::<
        ChaCha20UairBp, ChaCha20UairQx, FoldedZt, FoldedLc, 32, 16, 8, 4, UNCHECKED,
    >(
        &folded_params, &trace, CHACHA_NUM_VARS,
        &lookup_specs, &affine_specs, 2,
    );

    let full_proof = zinc_snark::pipeline::prove_hybrid_gkr_logup_8x_folded::<
        ChaCha20UairBp, ChaCha20UairQx, FoldedZt, FoldedLc, 32, 16, 8, 4, UNCHECKED,
    >(
        &folded_params, &trace, CHACHA_NUM_VARS,
        &lookup_specs, &affine_specs, 2,
    ...
    // Print prover pipeline timing breakdown.
    {
        ...
        eprintln!("\n── ChaCha20 8x-Folded Hybrid GKR c=2 Prover Pipeline Timing ──");
    ...
    // ── Proof size ───────────────────────────────────────────────────
    {
        ...
        eprintln!("\n=== ChaCha20 8x-Folded Hybrid GKR c=2 Proof Size ===");
    ...
    group.bench_function("E2E/Prover (8x-Folded Hybrid GKR c=2)", |b| {
        b.iter_custom(|iters| {
    ...
    // ── 3. E2E Total Verifier ───────────────────────────────────────
    {
        ...
        // D=32→16→8→4
        let fold_half_sizes = &[16usize, 8, 4];
        let r = zinc_snark::pipeline::verify_classic_logup_full_folded::<
            ChaCha20UairBp, ChaCha20UairQx, FoldedZt, FoldedLc, 32, 4, UNCHECKED, _, _, _, _,
        >(
        ...
        let t = &r.timing;
        eprintln!("\n── ChaCha20 8x-Folded Verifier step timing ─────────────");
    ...
    group.bench_function("E2E/Verifier (8x-Folded Hybrid GKR c=2)", |b| {
        b.iter(|| {
            #[cfg(not(feature = "true-ideal"))]
            fn qx_ideal_closure(_: &IdealOrZero<ChaCha20QxIdeal>) -> zinc_snark::pipeline::TrivialIdeal {
                zinc_snark::pipeline::TrivialIdeal
...
criterion_group!(benches, chacha20_8x_folded_stepwise);
