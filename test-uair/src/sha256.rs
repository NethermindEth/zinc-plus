...
//! ## Out of scope
//!
//! - **Bit-poly / range lookups** — the lookup PIOP in
//!   `protocol/src/prover.rs` is stubbed, so bit-poly membership and carry
//!   ranges (`mu_a ∈ [0, 6]`, `mu_e ∈ [0, 5]`, `mu_W ∈ [0, 3]`) are not
//!   enforced. The witness generator respects these bounds via
//!   `debug_assert!`s.
