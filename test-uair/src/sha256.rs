    ...
    R: ConstSemiring + 'static,
{
    ...
    fn signature() -> UairSignature {
        ...
        // would ideally be range-checked via logup-GKR (Word{width:2}
        // for mu_W ∈ [0,3]; Word{width:3} for mu_a/mu_e ∈ [0,6]). The
        // current gkr-logup pipeline only handles `BitPoly` lookups on
        // *binary_poly* witness columns, so those Word lookups on int
        // columns aren't yet supported (return `LookupError::NotImplemented`).
        // Stubbed out for now — joins the existing lookup-gap stance for
        // free-witness columns. Multiplicity columns `W_M_W2` / `W_M_W3`
        // remain in the trace but are unused by the protocol.
