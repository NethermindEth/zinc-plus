...
use crypto_primitives::{ConstSemiring, FromWithConfig, PrimeField, Semiring};
use num_traits::Zero;
use rand::RngCore;
use zinc_poly::{
...
impl<F: PrimeField> Sha256Ideal<F> {
    /// Convert a `Sha256Ideal<R>` into `Sha256Ideal<F>` using `field_cfg`.
    /// `RotXw1` carries no data (it always means `(X^32 - 1)`); `RotX2`
    /// delegates to `DegreeOneIdeal::from_with_cfg`.
    pub fn from_with_cfg<R>(ideal_over_ring: &Sha256Ideal<R>, field_cfg: &F::Config) -> Self
    where
        R: Semiring,
        F: for<'a> FromWithConfig<&'a R>,
    {
        match ideal_over_ring {
            Sha256Ideal::RotX2(ideal) => {
                Sha256Ideal::RotX2(DegreeOneIdeal::from_with_cfg(ideal, field_cfg))
            }
            Sha256Ideal::RotXw1 => Sha256Ideal::RotXw1,
        }
    }
}
...
// The projection closure in `protocol/src/lib.rs` converts non-zero
// `IdealOrZero` variants into a `Sha256Ideal<F>`; zero ideals are filtered
// out before the closure runs, so this path never needs to represent the
// zero-ideal case.
    ...
    F: PrimeField,
{
    fn contains(&self, value: &DynamicPolynomialF<F>) -> bool {
        ...
        match self {
            Sha256Ideal::RotX2(ideal) => IdealOrZero::<DegreeOneIdeal<F>> {
                ideal_or_zero: Some(ideal.clone()),
            }
            .contains(value),
            Sha256Ideal::RotXw1 => IdealOrZero::<XnMinusOneIdeal<32>> {
                ideal_or_zero: Some(XnMinusOneIdeal::<32>),
            }
            .contains(value),
        }
    }
    ...
    R: ConstSemiring + 'static,
{
    ...
    fn signature() -> UairSignature {
        ...
        // SOUNDNESS GAP: the integer carries `mu_W`, `mu_a`, `mu_e`
        // would ideally be range-checked via logup-GKR. xn-gamma has no
        // lookup pipeline; multiplicity columns `W_M_W2` / `W_M_W3`
        // remain in the trace for layout parity but are unused.
        ...
        IFromR: Fn(&Self::Ideal) -> B::Ideal,
    {
        ...
        let ideal_rot_x2 = ideal_from_ref(&Sha256Ideal::<R>::RotX2(
            DegreeOneIdeal::new(R::ONE + R::ONE), // (X − 2)
        ));
