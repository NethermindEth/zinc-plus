...
// `ECDSA_FINAL_ROW` and `NUM_SHAMIR_ROUNDS` are referenced only by the
// `re_exports` sanity test below; allow unused imports to keep the
// re-export surface intact in non-test builds.
    ...
    R: EcdsaFpRing + From<u32>,
{
    ...
    fn signature() -> UairSignature {
        ...
        // xn-gamma has no lookup pipeline; SHA's `mu_W/mu_a/mu_e`
        // integer carries would ideally be range-checked. See sha256.rs.
        // Multiplicity cols stay in the trace for layout parity but are
        // unused.
        ...
        IFromR: Fn(&Self::Ideal) -> B::Ideal,
    {
        ...
        let ideal_rot_x2 = ideal_from_ref(&Sha256Ideal::<R>::RotX2(DegreeOneIdeal::new(
            R::ONE + R::ONE,
        )));
