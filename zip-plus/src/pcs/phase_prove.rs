...
impl<Zt: ZipTypes, Lc: LinearCode<Zt>> ZipPlus<Zt, Lc> {
    ...
    /// Prove an evaluation claim at a new point **without** the proximity
    /// test. Used by the batched-shift step: proximity was already established
    /// via [`Self::prove_f`], so here we only write the data needed for the
    /// verifier to check evaluation consistency at a second point.
    ///
    /// # Transcript layout
    /// ```text
    /// [per-poly alphas sampled]
    /// [q_0_combined_row written as F elements]
    /// ```
    ///
    /// The verifier counterpart is [`Self::verify_evaluate_only`].
    #[allow(clippy::arithmetic_side_effects)]
    pub fn prove_evaluate_only_f<F, const CHECK_FOR_OVERFLOW: bool>(
        transcript: &mut PcsProverTranscript,
        pp: &ZipPlusParams<Zt, Lc>,
        polys: &[DenseMultilinearExtension<Zt::Eval>],
        point: &[F],
        ...
        F::Modulus: Transcribable,
    {
        ...
        // q_0_combined_row[col] = Σ_i Σ_j q_0[j] * W_i[j][col]
        // where W_i[j][col] is the alpha-reduced entry at (row j, col) of poly i.
        let q_0_combined_row = polys_as_comb_r.iter().try_fold(
            vec![zero_f.clone(); row_len],
            |mut acc, poly_comb_r| -> Result<_, ZipError> {
                for (j, row) in poly_comb_r.chunks(row_len).enumerate() {
                    for (col, val_comb_r) in row.iter().enumerate() {
                        let val_f: F = val_comb_r.into_with_cfg(field_cfg);
                        let mut contrib = q_0[j].clone();
                        contrib *= &val_f;
                        acc[col] += &contrib;
                    }
                }
        ...
        let eval = MBSInnerProduct::inner_product::<UNCHECKED>(&q_0_combined_row, &q_1, zero_f)?;
