...
use zinc_utils::{
    ...
    mul_by_scalar::{MulByScalar, ScalarWideningMulByScalar},
    named::Named,
...
fn bench_prove<Lc: LinearCode<IntZipTypes256>, const P: usize>(
    group: &mut BenchmarkGroup<WallTime>,
) where
    F: for<'a> FromWithConfig<&'a <IntZipTypes256 as ZipTypes>::CombR>
        + for<'a> FromWithConfig<&'a i128>
        + for<'a> MulByScalar<&'a F>,
    <F as Field>::Inner: FromRef<<IntZipTypes256 as ZipTypes>::Fmod> + ConstTranscribable,
    <IntZipTypes256 as ZipTypes>::Eval: ProjectableToField<F>,
{
    let poly_size: usize = 1 << P;
    let row_len = poly_size;
    let linear_code = Lc::new(row_len);
    ...
    group.bench_function(
        format!(
            "Prove: Eval={}, Cw={}, Comb={}, poly_size=2^{P}, 1row, modulus=({} bits)",
            <IntZipTypes256 as ZipTypes>::Eval::type_name(),
            <IntZipTypes256 as ZipTypes>::Cw::type_name(),
            <IntZipTypes256 as ZipTypes>::Comb::type_name(),
...
fn zip_int256_benchmarks(c: &mut Criterion) {
    ...
    bench_prove::<Iprs10, 10>(&mut group);
