...
/// Benchmarks for F65537 IPRS Depth-3 configuration with rate 1/4.
/// All configurations use row_len = 4096 (2^12) with rate 1/4.
/// Matrix sizes: 2^1 x 2^12, 2^2 x 2^12, 2^3 x 2^12, 2^4 x 2^12, 2^5 x 2^12
fn zip_plus_benchmarks_iprs_f65537_depth3_rate1_4(c: &mut Criterion) {
    let mut group = c.benchmark_group("Zip+ IPRS F65537 Depth3 Rate1_4");

    // 2 rows x 4096 cols (2^1 x 2^12), poly_size=2^13
    commit::<BenchZipPlusTypes<i64, 32>, SomeIprsCodeF65537Base8Depth3Rate1_4<i64, 32>, 13>(&mut group);
    // 4 rows x 4096 cols (2^2 x 2^12), poly_size=2^14
    commit::<BenchZipPlusTypes<i64, 32>, SomeIprsCodeF65537Base8Depth3Rate1_4<i64, 32>, 14>(&mut group);
    // 8 rows x 4096 cols (2^3 x 2^12), poly_size=2^15
    commit::<BenchZipPlusTypes<i64, 32>, SomeIprsCodeF65537Base8Depth3Rate1_4<i64, 32>, 15>(&mut group);
    // 16 rows x 4096 cols (2^4 x 2^12), poly_size=2^16
    commit::<BenchZipPlusTypes<i64, 32>, SomeIprsCodeF65537Base8Depth3Rate1_4<i64, 32>, 16>(&mut group);
    // 32 rows x 4096 cols (2^5 x 2^12), poly_size=2^17
    commit::<BenchZipPlusTypes<i64, 32>, SomeIprsCodeF65537Base8Depth3Rate1_4<i64, 32>, 17>(&mut group);
...
criterion_group!(
    ...
    zip_plus_benchmarks_iprs_f65537_depth3_rate1_4
);
