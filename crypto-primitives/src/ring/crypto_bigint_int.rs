...
impl<const LIMBS: usize> Int<LIMBS> {
    ...
    /// Checked mixed-width multiplication. Returns `None` on overflow.
    pub fn checked_mul_narrow<const RHS_LIMBS: usize>(
        &self,
        rhs: &Int<RHS_LIMBS>,
    ) -> Option<Self> {
        let result: crypto_bigint::subtle::CtOption<crypto_bigint::Int<LIMBS>> =
            crypto_bigint::CheckedMul::checked_mul(&self.0, &rhs.0);
        Option::from(result).map(Self)
    }

    /// Wrapping mixed-width multiplication, discarding overflow.
    pub fn wrapping_mul_narrow<const RHS_LIMBS: usize>(
        &self,
        rhs: &Int<RHS_LIMBS>,
    ) -> Self {
