...
The  public columns $\col{PA\_B1}, \col{PA\_B2}$ are
\emph{not} natively constrained to $\{0,1\}$ by the framework.  As with the rest of the input vectors, we assume they are correctly populated. 
\end{remark}
...
(the doubled point) and an affine addend $(T_X, T_Y)$ from 
 $\{G, Q, G+Q\}$, the mixed addition
...
need not be enforced in-circuit.
...
matching the Zinc+ main paper $s_{\mathrm{reg}} \cdot R_a \cdot X_{\mathrm{mid}} \cdot H^2$
term.
\end{remark}
