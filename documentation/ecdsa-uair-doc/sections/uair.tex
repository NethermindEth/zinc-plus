...
section. Each trace cell is an $\F_p$ field element (i.e.\
$d_p = 1$, treating $\F_p[X]$ at degree $0$).
...
Because every column is a native $\F_p$ field element and the
proving field equals the $\secp$ base prime, no range-check or
lookup constraint is required. This is the principal reason the
ECDSA UAIR has no lookup PIOP attached and no $\bitpoly{32}$
columns: arithmetic over $\F_p$ is direct.
\end{remark}
