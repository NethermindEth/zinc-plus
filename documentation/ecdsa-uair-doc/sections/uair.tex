...
Tables \ref{tab:pub-cols} and~\ref{tab:wit-cols} list the $13$ public
...
\midrule
\multicolumn{3}{l}{\textit{Public columns ($13$ — entire ECDSA UAIR is public-input--gated)}} \\[1pt]
...
\bottomrule
\end{tabular}
\caption{Public columns ($13$ total). All are $\F_p$-typed (i.e.\ \texttt{Int<5>}-stored, see \Cref{sec:cols}); the four selectors plus the bit pair are boolean. The boolean / constant-across-rows structural properties are intended to be discharged by verifier-side direct inspection of \texttt{public\_trace} (see soundness obligation in \Cref{rem:addend-cost}).}
\label{tab:pub-cols}
\end{table}
