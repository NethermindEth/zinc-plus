...
and univariate skip; \zinc{} runs the Hadamard discharge of
...
$16$\,GB), non-zero-knowledge, at the matched commitment rate $1/4$ and
with the security parameters each system ships ($t=987$ openings for
\zinc{}; $96$-bit for \binius{}). Both provers are measured \emph{CPU-only}
(rayon\,$+$\,SIMD), so the prover comparison is core-for-core; \zinc{}'s
optional Metal GPU commit path offloads the commit phase but shows no net
prover-latency win at the measured scales (\Cref{tab:scaling-gpu}), so the
CPU basis is the like-for-like one.
\binius{} proves a full padded preimage (length-binding in-circuit);
\zinc{}'s harness proves chained compressions only --- slightly less work,
which mildly flatters \zinc{}. Absolute times are thermally sensitive
($\sim$$10\%$ run-to-run on the prover); the ratios and the per-phase
\emph{shapes} are far more stable, and it is those we lean on.
\end{remark}
