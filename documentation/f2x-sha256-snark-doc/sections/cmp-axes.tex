...
each axis, and each axis decides one headline outcome. The prover is a
near-tie because it is dominated by content both systems share (the
$\AND$ discharge and the commitment encode), not by any single axis.}
\label{tab:axes}
\end{table}
...
\zinc{} keeps the witness bit-packed and runs the oblong discharge in the
...
\subsection{Why the prover is a tie}
...
roughly cancels at the current optimisation level. The per-phase
decomposition in \Cref{sec:results} shows this cancellation explicitly,
and \Cref{sec:analysis} argues that at scale the cancellation tips
slightly in \zinc{}'s favour.
