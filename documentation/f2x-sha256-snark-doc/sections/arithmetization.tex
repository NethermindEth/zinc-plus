...
\begin{itemize}[leftmargin=2em]
  ...
  \item \emph{Primary witness} (committed): the state columns
    ...
    the two $\AND$ product columns ($\bp{u}_{\mathrm{ch}}$ for $\Ch$ and the
    $\Maj$ product); the round intermediates $T_1,T_2$;
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \emph{Hadamard constraints} (the carry Hadamards of (b) and the
    ...
    operands bound to the commitment through the second projection $\psiz$.
\end{itemize}
