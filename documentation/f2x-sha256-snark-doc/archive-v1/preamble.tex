...
% The cell projection: evaluation at the random point alpha. There is ONE
% projection (X <- alpha); only the basis in which the witness bits are read
% as a polynomial differs -- monomial (ev^m) or Lagrange (ev^L). The old
% \psi_alpha / \psi_z notation is retired in favour of this explicit form;
% the macro names \psia/\psiz are kept as the monomial/Lagrange readings.
\newcommand{\psia}{\operatorname{ev}^{\mathrm{m}}_{\alpha}}   % monomial reading: ev^m_alpha(w) = w(alpha)
\newcommand{\psiz}{\operatorname{ev}^{\mathrm{L}}_{\alpha}}   % Lagrange reading: ev^L_alpha(w) = L(w)(alpha)
