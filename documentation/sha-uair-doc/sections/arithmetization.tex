...
$B_1,B_2,B_3$ of the Zinc+ main paper Table~9 (each storing one $\Q[X]$ affine
...
each per-bit slice of these residuals into $\{0,1\}$.
...
which is in $\{0,-1\}$ per coefficient — bit-invalid.
\item $r_{\mathrm{maj}}$ at $k = n-2$ reduces to $a[k]+a[k+1]$, which
is in $\{0,1,2\}$ per coefficient — also bit-invalid.
\end{itemize}
...
We place the compensator columns as public columns, with the verifier checking that their entries are zero for all $k<n-2$. 

In a future iteration of the implementation, we will use a Whirlaway-like (\url{https://github.com/leanEthereum/leanMultisig/blob/main/minimal_zkVM.pdf}) approach to forward shifts of col
