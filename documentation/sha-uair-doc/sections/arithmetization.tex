...
\emph{tail correctors} discussed below. The booleanity sumcheck pins
...
\end{itemize}
The public correctors are therefore non-zero only on those two tail
rows, with values
...
\end{equation}
Plugging \eqref{eq:correctors} into \eqref{eq:rch2} and \eqref{eq:rmaj}
recovers $\{0,1\}$-valued residuals on every row. Both correctors are
