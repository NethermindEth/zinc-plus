...
binary-polynomial columns and $11$ integer columns of our trace,
...
\textbf{Column} & \textbf{Role} & \textbf{Lookup set $\subseteq$ Domain} \\
\midrule
\multicolumn{3}{l}{\textit{Public integer columns ($6$)}} \\[1pt]
$S_{\mathrm{INIT-PREFIX}}$  & Selector: $1$ on each of the $N+1$ init-prefix blocks, else $0$ & $\{0,1\} \subseteq \Q$ \\
$S_{\mathrm{FF}}$           & Selector: $1$ on the $N$ junction windows, else $0$. Doubles as the C\ref{con:ff-a}/C\ref{con:ff-e} compensator-zero selector (C\ref{con:cz-ff-a}, C\ref{con:cz-ff-e}). & $\{0,1\} \subseteq \Q$ \\
...
$S_{\mathrm{ACTIVE-SCHED}}$ & Selector: $1$ on the $N$ schedule-active windows $[\,68i,\,68i+48\,)$, else $0$. Reads pin $\col{PA\_C\_C7}$ to $0$ via C\ref{con:cz-c7}. & $\{0,1\} \subseteq \Q$ \\
$S_{\mathrm{ACTIVE-UPD}}$   & Selector: $1$ on the $N$ update-active windows $[\,68i,\,68i+64\,)$, else $0$. Reads pin $\col{PA\_C\_C8}, \col{PA\_C\_C9}$ to $0$ via C\ref{con:cz-c8}, C\ref{con:cz-c9}. & $\{0,1\} \subseteq \Q$ \\
\midrule
\multicolumn{3}{l}{\textit{Witness integer columns ($5$)}} \\[1pt]
$\col{PA\_C\_C7}$           & Compensator for C\ref{con:c7}; zero on schedule-active rows (in-circuit-pinned by C\ref{con:cz-c7}), arbitrary on inactive rows & $\Q$ \\
$\col{PA\_C\_C8}$           & Compensator for C\ref{con:c8}; zero on update-active rows (in-circuit-pinned by C\ref{con:cz-c8}), arbitrary on inactive rows & $\Q$ \\
$\col{PA\_C\_C9}$           & Compensator for C\ref{con:c9}; zero on update-active rows (in-circuit-pinned by C\ref{con:cz-c9}), arbitrary on inactive rows & $\Q$ \\
$\col{PA\_C\_FF\_A}$        & Compensator for C\ref{con:ff-a}; zero on junction-window rows (in-circuit-pinned by C\ref{con:cz-ff-a}), arbitrary on inactive rows & $\Q$ \\
$\col{PA\_C\_FF\_E}$        & Compensator for C\ref{con:ff-e}; zero on junction-window rows (in-circuit-pinned by C\ref{con:cz-ff-e}), arbitrary on inactive rows & $\Q$ \\
\bottomrule
\end{tabularx}
\caption{Integer columns ($11$ total: $6$ public, $5$ witness). The five $\Q$-valued compensator columns $\col{PA\_C\_*}$ are committed witnesses; their zero-on-active-row property is enforced in-circuit by the assert-zero constraints C\ref{con:cz-c7}--C\ref{con:cz-ff-e} (each of the form $\col{PA\_C\_*}\rowidx{t} \cdot S_{\mathrm{ACTIVE-*}}\rowidx{t} = 0$), not by verifier-side public-trace inspection.}
\label{tab:int-cols}
\end{table}
...
inflate the constraint's effective degree. Why $\Q$-valued: on
inactive rows, $\mathrm{inner}(2)$ can be any rational, so the
compensator must range over all of $\Q$.

The compensator columns are committed \emph{witness} columns. Their
zero-on-active-row property is pinned in-circuit by the assert-zero
constraints C\ref{con:cz-c7}--C\ref{con:cz-ff-e}, each of the form
$\col{PA\_C\_*}\rowidx{t} \cdot S_{\mathrm{ACTIVE-*}}\rowidx{t} = 0$
where $S_{\mathrm{ACTIVE-*}}$ is the public selector that takes the
value $1$ on the constraint's active rows and $0$ elsewhere. The
selector is shared across constraints whose active ranges coincide:
$S_{\mathrm{ACTIVE-SCHED}}$ pins $\col{PA\_C\_C7}$,
$S_{\mathrm{ACTIVE-UPD}}$ pins both $\col{PA\_C\_C8}$ and
$\col{PA\_C\_C9}$, and $S_{\mathrm{FF}}$ (already needed elsewhere)
pins both $\col{PA\_C\_FF\_A}$ and $\col{PA\_C\_FF\_E}$.

The pinning constraints C\ref{con:cz-c7}--C\ref{con:cz-ff-e} are
polynomial-degree~$2$ (witness compensator $\times$ public selector),
but they live in the zero ideal, so they do not enter the projected
sum-check's effective-degree count and MLE-first eligibility is
preserved.

\paragraph{Tail compensators
$\col{PA\_R\_CH2\_COMP}, \col{PA\_R\_MAJ\_COMP}$ (bit-polynomial-typed, public).}
...
These remain \emph{public} columns: the verifier reads them from
\texttt{public\_trace} and discharges the zero-on-inner-rows property
by direct row-wise inspection (\Cref{cons:public-checks}). The
$\bitpoly{32}$ structural property is type-enforced.
\end{remark}
...
checks begin. These structural checks pin the public selectors,
round constants, and tail compensators row-wise without consuming
...
\begin{itemize}[leftmargin=2em]
  ...
  \item Every cell of every public integer column lies in its
    ...
    $\{0,1\}$ for the $S$-selectors and $[0..2^{32}-1]$ for
    $\col{PA\_K}$.
  ...
  \item Selector value pattern: each of $S_{\mathrm{INIT-PREFIX}},
    S_{\mathrm{FF}}, S_{\mathrm{MSG-INIT}},
    S_{\mathrm{ACTIVE-SCHED}}, S_{\mathrm{ACTIVE-UPD}}$ takes the
    value $1$ on its declared support set (\Cref{tab:int-cols}) and
    $0$ elsewhere.
\end{itemize}
...
The active-row compensator columns
$\col{PA\_C\_C7}, \col{PA\_C\_C8}, \col{PA\_C\_C9}, \col{PA\_C\_FF\_A}, \col{PA\_C\_FF\_E}$
are no longer in this list: they have been moved to the witness
section, and their zero-on-active-row property is now enforced
in-circuit by the assert-zero constraints
C\ref{con:cz-c7}--C\ref{con:cz-ff-e}.
...
Every row             & $[0, n)$                                  & C\ref{con:mu-zero}, C\ref{con:cz-c7}, C\ref{con:cz-c8}, C\ref{con:cz-c9}, C\ref{con:cz-ff-a}, C\ref{con:cz-ff-e}, C\ref{con:lkup-uef}, C\ref{con:lkup-uneg}, C\ref{con:lkup-maj} \\
\bottomrule
\end{tabular}
\end{center}
...
\paragraph{In-circuit compensator-zero pinning.}

For every row $t \in [0, n)$, the witness compensator columns of
\Cref{tab:int-cols} are pinned to $0$ on each constraint's active
row range by multiplying by the corresponding public selector and
asserting the zero ideal:
\begin{align}
\col{PA\_C\_C7}\rowidx{t}    \cdot S_{\mathrm{ACTIVE-SCHED}}\rowidx{t} &= 0, \tag{C18}\label{con:cz-c7}\\
\col{PA\_C\_C8}\rowidx{t}    \cdot S_{\mathrm{ACTIVE-UPD}}\rowidx{t}   &= 0, \tag{C19}\label{con:cz-c8}\\
\col{PA\_C\_C9}\rowidx{t}    \cdot S_{\mathrm{ACTIVE-UPD}}\rowidx{t}   &= 0, \tag{C20}\label{con:cz-c9}\\
\col{PA\_C\_FF\_A}\rowidx{t} \cdot S_{\mathrm{FF}}\rowidx{t}           &= 0, \tag{C21}\label{con:cz-ff-a}\\
\col{PA\_C\_FF\_E}\rowidx{t} \cdot S_{\mathrm{FF}}\rowidx{t}           &= 0. \tag{C22}\label{con:cz-ff-e}
\end{align}
On the active rows the public selector evaluates to $1$, so the
constraint forces the corresponding compensator cell to $0$ and the
ambient $(X-2)$ membership of C\ref{con:c7}/C\ref{con:c8}/C\ref{con:c9}/C\ref{con:ff-a}/C\ref{con:ff-e}
becomes a genuine SHA-recurrence/feed-forward equality on those
rows; on inactive rows the selector is $0$ and the compensator is
free to absorb $-\mathrm{inner}(2)$ (\Cref{rem:what-is-comp}).

Each of C\ref{con:cz-c7}--C\ref{con:cz-ff-e} is a constraint in the
zero ideal (strict equality), of polynomial-degree~$2$ in the trace
MLEs (witness compensator $\times$ public selector). Because the
zero ideal is excluded from the projected sum-check's
effective-degree count (same accounting as C\ref{con:c10}/C\ref{con:c11}/C\ref{con:msg-init}),
these constraints do not affect MLE-first eligibility.
...
\begin{align}
\bp{W_e}^{\downarrow 2}\rowidx{t} + \bp{W_e}^{\downarrow 1}\rowidx{t} - 2\,\bp{W_{u_{ef}}}^{\downarrow 2}\rowidx{t} &\in \bitpoly{32}, \tag{C23}\label{con:lkup-uef}\\
\bp{W_e}^{\downarrow 2}\rowidx{t} - \bp{W_e}\rowidx{t} + 2\,\bp{W_{u_{\neg e,g}}}^{\downarrow 2}\rowidx{t} + 2\,\col{PA\_R\_CH2\_COMP}\rowidx{t} &\in \bitpoly{32}, \tag{C24}\label{con:lkup-uneg}\\
\bp{W_a}\rowidx{t} + \bp{W_a}^{\downarrow 1}\rowidx{t} + \bp{W_a}^{\downarrow 2}\rowidx{t} - 2\,\bp{W_{\Maj}}^{\downarrow 2}\rowidx{t} - 2\,\col{PA\_R\_MAJ\_COMP}\rowidx{t} &\in \bitpoly{32}, \tag{C25}\label{con:lkup-maj}
\end{align}
