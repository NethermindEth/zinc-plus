...
| 3 | Evaluation projection `ψ_α` at `X = α ∈ GF(2^128)` | `GF(2^128)` after step |
| 4 | Combined CPR + lookup multi-degree sumcheck over `GF(2^128)` | `GF(2^128)` |
...
> a random element alpha from a fixed field extension of size 2^128 of
> F_2, and replace X by alpha. This way, our constraints are now over
> the binary field F_{2^128}. Now we run the sumchecks and everything
> (the rest of the protocol) as usual, over F_{2^128}. After that, we
> are left with MLE evaluation claims of the columns projected onto
> F_{2^128}. I will explain how to prove those in a next iteration of
...
**Commit 310ce0b — GF(2^128) field type + evaluation map.**
`BinaryFieldGF128` (NIST FIPS 186-2 reduction polynomial) with
Add/Sub/Mul/Inv/Pow and `a^{2^128} = a` verified by test. Plus
...
- **Coefficient field**: `GF(2^128)`. Random challenges are drawn
  from `GF(2^128)`.
- **Polynomial ring for the IC**: `GF(2^128)[X]`. The constraint
...
- **Lifting the trace**: every committed cell starts in `F_2[X]`
  (specifically `F_2[X]<32>`). `F_2 ⊂ GF(2^128)` is the trivial
  inclusion, so each `F_2[X]<32>` cell embeds canonically into
  `GF(2^128)[X]` — coefficient 0 stays `GF(2^128)::zero()`,
  coefficient 1 stays `GF(2^128)::one()`. No randomness is used in
  this lift; it's a representation change.
- **IC output point**: lives in `GF(2^128)^{num_vars}` (the MLE
  hypercube of the witnesses, but with GF(2^128)-valued challenges).

Steps 2 and 3 are *not* the same step and do not commute: step 2's
output is still in `GF(2^128)[X]` (polynomials in `X`), step 3 then
specializes `X = α` to land in `GF(2^128)`. This mirrors the original
protocol structure (`F_q[X] → F_q` is the same kind of `X → α`
substitution), just with `F_q` replaced by `GF(2^128)` and the prior
`Z[X] → F_q[X]` step muted because we never had `Z` to begin with.

The arithmetic primitives the IC needs in this regime:
- `GF(2^128) + GF(2^128) → GF(2^128)`: have it (XOR).
- `GF(2^128) × GF(2^128) → GF(2^128)`: have it (clmul + reduce mod
  pentanomial).
- `F_2[X]<32> · GF(2^128) → GF(2^128)[X]<32>`: scalar multiply a
  binary cell by a `GF(2^128)` coefficient — a 32-element vector of
  `GF(2^128)` values (zero where the bit is unset, the scalar where
  it's set). Cheap.
- `GF(2^128)[X] + GF(2^128)[X]` and `GF(2^128)[X] × GF(2^128)`: bag
  of `GF(2^128)` ops. Need a representation, probably a thin
  `DensePolynomial<GF(2^128), D>` — or `DynamicPolynomialF<...>`
...
axis) but swaps every `F::*` primitive for the `GF(2^128)` equivalent
and every `DynamicPolynomialF<F>` for a polynomial type over
`GF(2^128)`.

### Step 3 — Evaluation projection (X → α)

Sample `α ← Transcript::get_challenge() ∈ GF(2^128)` and substitute
`X = α` in every `GF(2^128)[X]`-valued object that survived step 2:

- Each trace cell (originally `F_2[X]<32>`, used by step 2 via the
  trivial `F_2 ⊂ GF(2^128)` coefficient lift) → a single
  `GF(2^128)` value. Already implemented per-cell as
  `eval_f2_poly_d32_at` / `eval_f2_u64_poly_at` /
  ...
  but with `F = GF(2^128)` and dense MLEs over `GF(2^128)`).
- The IC's emitted `GF(2^128)[X]` polynomials (constraint claims,
  evaluation-point coordinates, etc.) → `GF(2^128)` values via
  ordinary polynomial-at-a-point evaluation. The existing
  `EvaluatablePolynomial::evaluate_at_point` machinery should fit
  once the coefficient type is `GF(2^128)`.

After step 3, every value the verifier and prover are reasoning about
is a single `GF(2^128)` element — no polynomial-in-X anywhere.

### Step 4 — Sumcheck over GF(2^128)

The existing [`MultiDegreeSumcheck`](../piop/src/sumcheck.rs) binds
`F: PrimeField`. `BinaryFieldGF128` is a `Field` but not a
`PrimeField` (no integer modulus). Two options:

- **(A) Synthetic PrimeField impl.** Give `BinaryFieldGF128` a
...
- **(B) Narrow the trait bound.** Audit which `PrimeField` methods
  ...
  trait that both `PrimeField` and `BinaryFieldGF128` satisfy. Then
...
(A) chosen, `BinaryFieldGF128: PrimeField` implemented. Trait
bounds resolve — `multi_degree_compiles_against_gf128` passes.
...
holds. For `GF(2^128)` it does **not**: the only ring homomorphism
`Z → GF(2^128)` factors through `Z/2Z`, so
...
to GF(2^128) or to our `From<u64>` choice. Resolving it requires
either:
...
The runtime test `multi_degree_against_gf128_runtime` in
...
the columns projected onto F_{2^128}. I will explain how to prove
...
1. **Decide (A) vs (B)** for the `PrimeField` trait-bound question.
   ...
   `PrimeField` impl on `BinaryFieldGF128`; (B) = carve out a
   `ProtocolField` trait and narrow piop's bound. See "Step 4" above.

2. **`GF(2^128)[X]` polynomial type.** A `DensePolynomial<GF(2^128),
   D>` (or `DynamicPolynomialF<GF(2^128)>` if the dynamic-degree
   variant is needed for the IC). Should fall out of (A)/(B)
   automatically once `GF(2^128)` satisfies the necessary trait
   surface; if not, add a concrete impl block.

3. **F_2[X] → GF(2^128)[X] coefficient lift.** Trivial map: each
   `Boolean` coefficient becomes `GF(2^128)::zero()` or
   `GF(2^128)::one()`. Small helper `lift_f2_poly_to_gf128_poly`.

4. **Port `IdealCheckProtocol::prove_{combined,linear,hybrid}` to a
   `_f2` variant** that takes lifted-trace and `GF(2^128)`
   challenges. Mirrors the existing combinatorial structure;
   every `F`-typed primitive becomes `GF(2^128)` and every
   `DynamicPolynomialF<F>` becomes the corresponding
   `GF(2^128)[X]` type from step 2.
...
5. **Wire trace projection** (the eval-at-α already implemented
   ...
   trace cells and any `GF(2^128)[X]` objects the IC emitted.

6. **Hook the existing sumcheck to `GF(2^128)`** — assuming step 1
   chose (A) or (B) appropriately. The sumcheck's internal `F::*`
   primitives map onto `GF(2^128)` field ops.
