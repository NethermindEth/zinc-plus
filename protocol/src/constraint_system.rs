...
pub trait ConstraintSystem: Clone + Debug {
    ...
    ///
    /// Default `None`: the substrate samples a fresh random
    /// $\Omega(\lambda)$-bit prime $q_0$ (the UAIR / $\mathbb{Q}$ path,
    /// where the random prime is what makes the integer-relation projection
    /// sound). A frontend whose relation is **native over a specific prime
    /// field** — e.g. R1CS over a fixed $F$ (the Zinc+ "add-on" / $R =
    /// \mathbb{F}_q$ case) — returns `Some(cfg)`, and the substrate uses it
    /// as the working field ($q_0$, and $q''$ since it aliases when there
    /// are no declared primes) instead of sampling.
    ///
