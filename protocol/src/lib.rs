...
/// 2× (decomposes each `Int<INT_LIMBS>` cell as
/// `lo + 2^(INT_HALF_LIMBS · 64) · hi`, with `lo, hi : Int<INT_HALF_LIMBS>`).
///
/// Concretely: the int Zip+ instance commits the **split** int trace at
/// length `2n` with `Eval = Int<INT_HALF_LIMBS>`, and the protocol opens it
/// at the same extended point `(r_0 ‖ γ)` as the binary fold. The verifier
/// mirrors binary's `(1−γ) c1 + γ c2` reconstruction with `alpha_stride = 1`
/// (since `IntZt::Cw` is scalar).
///
/// The trait constrains `Self::Int = Int<INT_LIMBS>` so that
/// [`compute_int_fold_lifted_evals`] and [`split_int_columns`] can operate
/// on the trace cells.
...
>: Clone + Debug
{
    ...
    /// Zip+ types for the split binary trace columns
    /// (`Eval = BinaryPoly<HALF_D>`, identical bound to [`FoldedZincTypes`]).
    type BinaryZt: ZipTypes<
            Eval = BinaryPoly<HALF_D>,
            Chal = Self::Chal,
            Pt = Self::Pt,
    ...
    /// Zip+ types for the arbitrary polynomial trace columns (unchanged from
    /// the unfolded path: degree-`D` polynomials over `Int<INT_LIMBS>`).
    type ArbitraryZt: ZipTypes<
...
/// `BinaryPoly<D> → BinaryPoly<QUARTER_D>` (D → D/4) AND folds int by 4×
/// via `Int<INT_LIMBS> → Int<INT_QUARTER_LIMBS>` quarters
/// (`v = q_0 + 2^64·q_1 + 2^128·q_2 + 2^192·q_3`).
///
/// The int Zip+ commits the length-`4n` quarter-witness column at
/// `Eval = Int<INT_QUARTER_LIMBS>`, and the protocol opens it at the
/// doubly-extended point `(r_0 ‖ γ_1 ‖ γ_2)` — same point as the 4×
/// binary fold. The verifier mirrors with the 4-block algebra
/// `(1−γ_1)(1−γ_2) c[0] + γ_1(1−γ_2) c[2] + (1−γ_1)γ_2 c[1] + γ_1γ_2 c[3]`,
...
>: Clone + Debug
{
    ...
    /// Zip+ types for the **twice-split** binary trace columns
    ...
    /// Zip+ types for the **quartered** integer trace columns
    /// (`Eval = Int<INT_QUARTER_LIMBS>`).
    type IntZt: ZipTypes<
...
/// 4× int-fold counterpart of [`compute_int_fold_lifted_evals`]:
/// produces 4-coefficient bar_us per int column for the chained
/// 1×→2× (i.e. 4×) int fold.
///
/// For each int column j with cells `v[i] : Int<H>`, the four
/// coefficients are MLE evaluations at `point` of the four
/// 64-bit-quarter columns under the signed decomposition
/// `v = q_0 + 2^64·q_1 + 2^128·q_2 + 2^192·q_3` (where
/// `q_0,q_1,q_2` are unsigned single-limb projections and `q_3` is
/// the signed arithmetic shift):
///
/// ```text
/// bar_u_j.coeffs[0] = sum_b eq(b, point) * lift_F(q_0(v[b]))
/// bar_u_j.coeffs[1] = sum_b eq(b, point) * lift_F(q_1(v[b]))
/// bar_u_j.coeffs[2] = sum_b eq(b, point) * lift_F(q_2(v[b]))
/// bar_u_j.coeffs[3] = sum_b eq(b, point) * lift_F(q_3(v[b]))
/// ```
///
/// Coefficient ordering matches the binary 4× fold's verifier
/// formula at [`crate::verifier::verify_folded_4x`]: the original
/// column's lifted eval is recoverable as
/// `c[0] + 2^64·c[1] + 2^128·c[2] + 2^192·c[3]`, and the verifier's
/// `(1−γ_1)(1−γ_2) c[0] + γ_1(1−γ_2) c[2] + (1−γ_1)γ_2 c[1] +
/// γ_1·γ_2 c[3]` reconstruction follows the chained-split layout in
/// [`zip_plus::pcs::folding::split_int_column_4x`].
///
/// `Q` must be `>= 2` (signed `Int<1>` overflows on quarters with
/// magnitude `>= 2^63`).
#[allow(clippy::arithmetic_side_effects)]
        ...
        + for<'a> FromWithConfig<&'a crypto_primitives::crypto_bigint_int::Int<Q>>,
{
    ...
    assert!(
        Q >= 2,
        "compute_int_fold_4x_lifted_evals: Q ({Q}) must be >= 2"
    );
    assert!(
        H >= 4,
        "compute_int_fold_4x_lifted_evals: H ({H}) must be >= 4"
    );
    let eq_table = zinc_poly::utils::build_eq_x_r_vec(point, field_cfg)
...
mod tests {
    ...
    // ── Int-fold variant of the ShaEcdsa types (for prove_folded_with_int_fold) ──
    //
    // Mirrors TestShaEcdsaZincTypes but: (a) the binary commitment is
    // over BinaryPoly<HALF_DEGREE_PLUS_ONE> (1× binary fold), and (b) the
    ...
    /// Split-int Zip+ for ShaEcdsa: Eval = Int<2> (the post-fold half),
    /// Cw widened by one limb, CombR over-provisioned.
    #[derive(Debug, Clone)]
    pub struct IntZipTypesShaEcdsaSplit {}
    impl ZipTypes for IntZipTypesShaEcdsaSplit {
        const NUM_COLUMN_OPENINGS: usize = NUM_COL_OPENINGS_FOR_REP;
        type Eval = Int<INT_HALF_LIMBS>;
        type Cw = Int<{ INT_HALF_LIMBS + 1 }>;
        type Fmod = Uint<FIELD_LIMBS>;
        type PrimeTest = MillerRabin;
        type Chal = i128;
        type Pt = i128;
    ...
    #[derive(Clone, Debug)]
    struct TestShaEcdsaIntFoldZincTypes;

    impl
        IntFoldedZincTypes<DEGREE_PLUS_ONE, HALF_DEGREE_PLUS_ONE, EC_FP_INT_LIMBS, INT_HALF_LIMBS>
        for TestShaEcdsaIntFoldZincTypes
    {
        type Chal = i128;
        type Pt = i128;
        type Fmod = Uint<FIELD_LIMBS>;
        ...
        type ArbitraryZt = ArbPolyZipTypesShaEcdsa;
        type IntZt = IntZipTypesShaEcdsaSplit;
    ...
    #[allow(clippy::type_complexity)]
    fn setup_int_folded_pp_sha_ecdsa(
        num_vars: usize,
    ) -> (
        ZipPlusParams<
            <TestShaEcdsaIntFoldZincTypes as IntFoldedZincTypes<
                DEGREE_PLUS_ONE,
                HALF_DEGREE_PLUS_ONE,
                EC_FP_INT_LIMBS,
                INT_HALF_LIMBS,
    ...
    //
    // Mirrors TestShaEcdsaIntFoldZincTypes but with twice-split binary
    // (BinaryPoly<8>) and quartered int (Int<2>) commitments.

    const QUARTER_DEGREE_PLUS_ONE_4X_INT: usize = 8;
    const INT_QUARTER_LIMBS: usize = 2;

    #[derive(Debug, Clone)]
    pub struct BinPolyZipTypesShaEcdsaQuarter {}
    impl ZipTypes for BinPolyZipTypesShaEcdsaQuarter {
        const NUM_COLUMN_OPENINGS: usize = NUM_COL_OPENINGS_FOR_REP;
        type Eval = BinaryPoly<QUARTER_DEGREE_PLUS_ONE_4X_INT>;
        type Cw = DensePolynomial<i64, QUARTER_DEGREE_PLUS_ONE_4X_INT>;
        type Fmod = Uint<FIELD_LIMBS>;
        type PrimeTest = MillerRabin;
        type Chal = i128;
        type Pt = i128;
        type CombR = Int<{ EC_FP_INT_LIMBS * 4 }>;
        type Comb = DensePolynomial<Self::CombR, QUARTER_DEGREE_PLUS_ONE_4X_INT>;
        type EvalDotChal = BinaryPolyInnerProduct<Self::Chal, QUARTER_DEGREE_PLUS_ONE_4X_INT>;
        type CombDotChal = DensePolyInnerProduct<
            Self::CombR,
            Self::Chal,
            Self::CombR,
            MBSInnerProduct,
            QUARTER_DEGREE_PLUS_ONE_4X_INT,
        >;
        type ArrCombRDotChal = MBSInnerProduct;
    }

    /// Quartered-int Zip+: `Eval = Int<2>` (post-4×-fold quarter,
    /// 128-bit storage holding 64 bits of magnitude with sign-bit
    /// headroom), `Cw = Int<3>`.
    #[derive(Debug, Clone)]
    pub struct IntZipTypesShaEcdsaQuarter {}
    impl ZipTypes for IntZipTypesShaEcdsaQuarter {
        const NUM_COLUMN_OPENINGS: usize = NUM_COL_OPENINGS_FOR_REP;
        type Eval = Int<INT_QUARTER_LIMBS>;
        type Cw = Int<{ INT_QUARTER_LIMBS + 1 }>;
        type Fmod = Uint<FIELD_LIMBS>;
        type PrimeTest = MillerRabin;
        type Chal = i128;
        type Pt = i128;
    ...
    /// 1×-folded ShaEcdsa round-trip with **int folding enabled**. Exercises
    /// `prove_folded_with_int_fold` / `verify_folded_with_int_fold`: binary
    /// witnesses are split into BinaryPoly<16> halves AND int witnesses are
    /// split into Int<2> halves under v = lo + 2^128 · hi. Prints the
    /// serialized proof size for comparison against the unfolded /
    /// binary-only-folded paths.
    #[test]
    fn test_e2e_sha_ecdsa_int_fold_round_trip() {
        use crate::prover::prove_folded_with_int_fold;
        use crate::verifier::verify_folded_with_int_fold;

        type ZtF = TestShaEcdsaIntFoldZincTypes;
        type U = ShaEcdsaUair<ShaEcdsaInt>;

        let mut rng = rng();
        let pp = setup_int_folded_pp_sha_ecdsa(SHA_ECDSA_NUM_VARS);
        let trace = U::generate_random_trace(SHA_ECDSA_NUM_VARS, &mut rng);
        let sig = <U as Uair>::signature();
        let public_trace = trace.public(&sig);

        let proof = prove_folded_with_int_fold::<
            ZtF,
            U,
            F,
            DEGREE_PLUS_ONE,
        ...
        .expect("Folded-with-int-fold prover failed");

        let mut transcript = PcsProverTranscript::new_from_commitments(std::iter::empty());
        transcript.write(&proof).expect("Failed to serialize proof");
        let serialized_len = transcript.stream.get_ref().len();
        println!(
            "int-fold ShaEcdsa proof size: {} bytes ({} KiB)",
            serialized_len,
        ...
        .expect("Verifier rejected an honest int-fold ShaEcdsa proof");
    }

    /// 4×-folded ShaEcdsa round-trip with **int folding enabled** —
    /// binary witnesses split into BinaryPoly<8> quarters AND int
    /// witnesses split into Int<2> quarters under
    /// `v = q_0 + 2^64·q_1 + 2^128·q_2 + 2^192·q_3`. Prints the proof
    /// size for comparison against the 1× int-fold path.
    #[test]
    fn test_e2e_sha_ecdsa_int_fold_4x_round_trip() {
        ...
        let proof = prove_folded_4x_with_int_fold::<
            ...
            CHECKED,
        >(&pp, &trace, SHA_ECDSA_NUM_VARS, project_scalar_fn)
        .expect("4x int-fold prover failed");

        let mut transcript = PcsProverTranscript::new_from_commitments(std::iter::empty());
        transcript.write(&proof).expect("Failed to serialize proof");
        let serialized_len = transcript.stream.get_ref().len();
        println!(
            "4x int-fold ShaEcdsa proof size: {} bytes ({} KiB)",
            serialized_len,
            serialized_len.div_ceil(1024),
        );
        let mut transcript = transcript.into_verification_transcript();
        ...
        .expect("Verifier rejected an honest 4x int-fold ShaEcdsa proof");
    }
