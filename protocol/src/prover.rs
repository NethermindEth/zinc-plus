...
use zip_plus::{
    pcs::{
        multi_zip::{MultiZip3, MultiZipHint3},
...
pub struct ProverBase<'a, Zt: ZincTypes<D>, U: Uair, F: PrimeField, const D: usize> {
    ...
    /// Set in step 0 when all three witness batches are non-empty: holds
    /// the per-instance encoded matrices and the *shared* Merkle tree.
    /// When `Some`, step 7 opens via [`MultiZip3::prove_f`] and writes one
    /// path per opening; when `None`, falls back to per-instance
    /// [`ZipPlus::prove_f`] (one path per opening per instance).
    multi_hint: Option<
        MultiZipHint3<
            <Zt::BinaryZt as ZipTypes>::Cw,
            <Zt::ArbitraryZt as ZipTypes>::Cw,
            <Zt::IntZt as ZipTypes>::Cw,
        >,
    >,
    ...
    F::Inner: ConstTranscribable,
{
    ...
    pub fn step0_commit<'a>(
        ...
        // Shared-Merkle path: if at least two of the witness batches are
        // non-empty, run a single MultiZip3::commit so those Zip+ instances
        // share one Merkle tree (and one path per column opening at step
        // 7). With <2 non-empty batches there's nothing to share — fall
        // back to the per-instance flow.
        let nonempty_count = (!witness_trace.binary_poly.is_empty()) as u8
            + (!witness_trace.arbitrary_poly.is_empty()) as u8
            + (!witness_trace.int.is_empty()) as u8;
        let use_multi = nonempty_count >= 2;

        let (hint_bin, hint_arb, hint_int, multi_hint, commitment_bin, commitment_arb, commitment_int) =
            if use_multi {
...
impl_with_type_bounds!(ProverLifted
{
    ...
    pub fn step7_pcs_open<const CHECK_FOR_OVERFLOW: bool>(
        ...
        if let Some(multi_hint) = &self.base.multi_hint {
            // Shared-Merkle path: one column-opening loop and one Merkle path
            // per opening across all three Zip+ instances.
            let _ = MultiZip3::<
                Zt::BinaryZt,
                Zt::ArbitraryZt,
                Zt::IntZt,
...
/// 1× folded prover that **also** folds int witness columns by 2× via the
/// `v = lo + 2^(INT_HALF_LIMBS · 64) · hi` decomposition. Mirrors
/// [`prove_folded`] but commits the int Zip+ instance over `Int<INT_HALF_LIMBS>`
/// halves at length `2n` and opens it at the same extended point
/// `(r_0 ‖ γ)` as the binary fold.
///
/// Lifted-evals for int columns are computed via
/// [`crate::compute_int_fold_lifted_evals`] (returns 2-coeff bar_us:
/// `[lo_eval_at_r0, hi_eval_at_r0]`). The verifier mirrors binary's
/// `(1−γ) c1 + γ c2` reconstruction with `alpha_stride = 1` since
/// `IntZt::Cw` is scalar.
#[allow(clippy::too_many_arguments, clippy::type_complexity)]
pub fn prove_folded_with_int_fold<
    ZtF,
    U,
    F,
    const D: usize,
...
where
    ...
    F::Modulus: ConstTranscribable + FromRef<ZtF::Fmod>,
{
    let (pp_bin_split, pp_arb, pp_int_split) = pp;
    let uair_signature = U::signature();
    let public_trace = trace.public(&uair_signature);
    let witness_trace = trace.witness(&uair_signature);

    // ── Step 0: Commit (split binary, arb, split int) ───────────────────
    let split_binary_witness: Vec<DenseMultilinearExtension<BinaryPoly<HALF_D>>> =
        zip_plus::pcs::folding::split_columns::<D, HALF_D>(&witness_trace.binary_poly);
    ...
    // ── Step 6: Lift-and-project + sample γ ─────────────────────────────
    // Compute lifted_evals as in the binary-only fold, then REPLACE the
    // int section with 2-coeff bar_us via `compute_int_fold_lifted_evals`
    // so the verifier can apply the fold algebra to ints too.
    ...
    let int_lifted_evals_2coeff: Vec<DynamicPolynomialF<F>> =
    ...
    for (i, bar_u) in int_lifted_evals_2coeff.into_iter().enumerate() {
    ...
    // ── Step 7: PCS open. Binary AND int use split commitments at r_0 ‖ γ
    let mut r0_ext = r_0.clone();
    r0_ext.push(gamma);
...
/// 4×-folded prover that **also** folds int witness columns by 4× via
/// `v = q_0 + 2^64 q_1 + 2^128 q_2 + 2^192 q_3` decomposition into
/// `Int<INT_QUARTER_LIMBS>` quarters. Mirrors [`prove_folded_4x`] but
/// commits the int Zip+ instance over quartered columns at length `4n`
/// and opens it at the same extended point `(r_0 ‖ γ₁ ‖ γ₂)` as the
/// binary 4× fold.
///
/// When ≥2 batches are non-empty (and arb is empty or has a matching
/// codeword length), routes the commit/open through
/// [`MultiZip3`](zip_plus::pcs::multi_zip::MultiZip3) for shared-Merkle
/// savings — one Merkle path per opening instead of three.
#[allow(clippy::too_many_arguments, clippy::type_complexity)]
pub fn prove_folded_4x_with_int_fold<
    ZtF,
    U,
    F,
    const D: usize,
...
//
// Folded prover (8× fold = 3 rounds, D → HALF_D → QUARTER_D → EIGHTH_D).
//
// Splits each `BinaryPoly<D>` witness column into a length-8n column of
// `BinaryPoly<EIGHTH_D>` entries (via three chained 2× splits), commits the
// thrice-split column, and opens it at the triply-extended PIOP point
// `(r_0 ‖ γ₁ ‖ γ₂ ‖ γ₃)`. Compared to 4× fold, each codeword element is half
// the size and the verifier's bilinear interpolation becomes trilinear.
//
// As with `prove_folded_4x`, the PIOP runs unchanged on the original
// `BinaryPoly<D>` trace; the verifier reconstructs the binary PCS `eval_f`
// from `witness_lifted_evals` coefficient octets (see `verify_folded_8x`).
//

/// Artifacts produced by the folded-8× commit step. Returned by
/// [`folded_8x_commit_witness`] and consumed by [`folded_8x_open_witness`]
/// at step 7.
#[allow(clippy::type_complexity)]
pub struct Folded8xCommittedArtifacts<ZtF, const D: usize, const EIGHTH_D: usize>
where
    ZtF: crate::FoldedZincTypes<D, EIGHTH_D>,
{
    pub split3: Vec<DenseMultilinearExtension<BinaryPoly<EIGHTH_D>>>,
    pub hint_bin_split: Option<ZipPlusHint<<ZtF::BinaryZt as ZipTypes>::Cw>>,
    pub commitment_bin: ZipPlusCommitment,
    pub hint_arb: Option<ZipPlusHint<<ZtF::ArbitraryZt as ZipTypes>::Cw>>,
    pub commitment_arb: ZipPlusCommitment,
...
/// Folded-8× witness commit: splits the binary witness three times
/// (D → HALF_D → QUARTER_D → EIGHTH_D) and commits all three trace components
/// in parallel via `cfg_join!`. Mirrors the commit work performed at the top
/// of [`prove_folded_8x`].
#[allow(clippy::type_complexity)]
pub fn folded_8x_commit_witness<
    ZtF,
    const D: usize,
    const HALF_D: usize,
    const QUARTER_D: usize,
...
>(
    ...
    witness_arbitrary_poly: &[DenseMultilinearExtension<<ZtF::ArbitraryZt as ZipTypes>::Eval>],
    witness_int: &[DenseMultilinearExtension<<ZtF::IntZt as ZipTypes>::Eval>],
) -> Result<Folded8xCommittedArtifacts<ZtF, D, EIGHTH_D>, ZipError>
where
    ZtF: crate::FoldedZincTypes<D, EIGHTH_D>,
{
    let (pp_bin_split3, pp_arb, pp_int) = pp;
    ...
    let split2: Vec<DenseMultilinearExtension<BinaryPoly<QUARTER_D>>> =
        zip_plus::pcs::folding::split_columns::<HALF_D, QUARTER_D>(&split1);
    drop(split1);
    // Third split: QUARTER_D → EIGHTH_D, length 4n → 8n.
    let split3: Vec<DenseMultilinearExtension<BinaryPoly<EIGHTH_D>>> =
        zip_plus::pcs::folding::split_columns::<QUARTER_D, EIGHTH_D>(&split2);
    drop(split2);

    let (res_bin, (res_arb, res_int)) = cfg_join!(
        commit_optionally(pp_bin_split3, &split3),
        commit_optionally(pp_arb, witness_arbitrary_poly),
        commit_optionally(pp_int, witness_int),
    );
    let (hint_bin_split, commitment_bin) = res_bin?;
...
/// Folded-8× PCS open: opens the split3 binary witness at
/// `r_0 ‖ γ₁ ‖ γ₂ ‖ γ₃`, plus arbitrary and int witnesses at `r_0`. Mirrors
/// step 7 of [`prove_folded_8x`].
#[allow(clippy::too_many_arguments, clippy::type_complexity)]
pub fn folded_8x_open_witness<
    ZtF,
    F,
    I,
    const D: usize,
...
>(
    ...
    artifacts: &Folded8xCommittedArtifacts<ZtF, D, EIGHTH_D>,
    witness_arb: &[DenseMultilinearExtension<<ZtF::ArbitraryZt as ZipTypes>::Eval>],
    witness_int: &[DenseMultilinearExtension<<ZtF::IntZt as ZipTypes>::Eval>],
    r_0: &[F],
    gamma1: F,
...
where
    ZtF: crate::FoldedZincTypes<D, EIGHTH_D>,
    <ZtF::ArbitraryZt as ZipTypes>::Eval: ProjectableToField<F>,
    BinaryPoly<EIGHTH_D>: ProjectableToField<F>,
    I: zinc_uair::ideal::Ideal,
    F: InnerTransparentField
        + FromPrimitiveWithConfig
        + for<'a> FromWithConfig<&'a ZtF::Int>
    ...
    F::Modulus: ConstTranscribable + FromRef<ZtF::Fmod>,
{
    ...
    // Binary opens the thrice-split column at point (r_0 ‖ γ₁ ‖ γ₂ ‖ γ₃).
    let mut r0_ext = r_0.to_vec();
    r0_ext.push(gamma1);
    r0_ext.push(gamma2);
    r0_ext.push(gamma3);
...
/// Folded-8× prover. See module docs at the top of this section.
#[allow(clippy::too_many_arguments, clippy::type_complexity)]
pub fn prove_folded_8x<
    ZtF,
    U,
    F,
    const D: usize,
...
/// Identical to [`prove_folded_8x`], but additionally returns a per-region
/// wall-time breakdown.
#[allow(clippy::too_many_arguments, clippy::type_complexity)]
pub fn prove_folded_8x_with_timings<
    ZtF,
    U,
    F,
    const D: usize,
...
{
    let mut timings = FoldedProveTimings::default();
    let proof = prove_folded_8x_inner::<
        ZtF,
        U,
        F,
        D,
    ...
    F::Modulus: ConstTranscribable + FromRef<ZtF::Fmod>,
{
    ...
    // ── Step 0: Commit (split binary thrice, regular arb/int) ───────────
    let _t = std::time::Instant::now();
    let artifacts = folded_8x_commit_witness::<ZtF, D, HALF_D, QUARTER_D, EIGHTH_D>(
        pp,
        &witness_trace.binary_poly,
        &witness_trace.arbitrary_poly,
        &witness_trace.int,
    ...
    // ── Step 1: Prime projection ────────────────────────────────────────
    // See `prove_folded_4x_inner` for the secp256k1 fixed-prime rationale.
    let _t = std::time::Instant::now();
    let field_cfg = crate::fixed_prime::secp256k1_field_cfg::<F, ZtF::Fmod>();
    let projected_scalars_fx = project_scalars::<F, U>(|s| project_scalar(s, &field_cfg));
    if let Some(t) = timings.as_mut() {
    ...
    // ── Step 6: Lift-and-project + sample γ₁, γ₂, γ₃ ────────────────────
    let _t_step6 = std::time::Instant::now();
    let lifted_evals =
        compute_lifted_evals::<F, D>(&r_0, &trace.binary_poly, &projected_trace, &field_cfg);
    let mut transcription_buf: Vec<u8> = vec![0; F::Inner::NUM_BYTES];
    ...
    // After absorbing lifted_evals, sample γ₁, γ₂, γ₃ in order — the verifier
    // replays in the same order before any PCS verify.
    let gamma1: F = {
        let g_chal: ZtF::Chal = pcs_transcript.fs_transcript.get_challenge();
        F::from_with_cfg(&g_chal, &field_cfg)
    };
    ...
    // ── Step 7: PCS open ────────────────────────────────────────────────
    let _t_step7 = std::time::Instant::now();
    folded_8x_open_witness::<ZtF, F, U::Ideal, D, EIGHTH_D, CHECK_FOR_OVERFLOW>(
        &mut pcs_transcript,
        pp,
        &artifacts,
        &witness_trace.arbitrary_poly,
