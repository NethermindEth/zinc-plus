...
#[doc(hidden)]
pub trait ProductionShaMixedHyraxPcs<Zt, F, const D: usize>: AffineRepr
where
    Zt: ZincTypes<D>,
    ...
    HyraxPCS<Self, DensePolyScalarLanes>: PCS<
            ...
            OpeningProof = Vec<u8>,
        >,
    HyraxPCS<Self, IntScalarLane>: PCS<
            F,
            Zt::Int,
            D,
            CommitmentKey = HyraxCommitmentKey<Self>,
...
impl<C, Zt, F, const D: usize> ProductionShaMixedHyraxPcs<Zt, F, D> for C
where
    Zt: ZincTypes<D>,
...
#[derive(Clone, Debug)]
pub struct ProductionShaMixedHyraxProof<C, F>
where
    C: AffineRepr,
    F: PrimeField,
{
    ...
    F: PrimeField,
{
    ...
    pub sumfold_basis: PreparedShaSumFoldBasis<F>,
}
    ...
    F: PrimeField,
{
        ...
        Self: Sized,
    {
        let (trace, public, witness_polys) =
            Self::project_production_sha_witness(shape, public_trace, witness_trace, field_cfg)?;
        Ok((trace, public, witness_polys, None))
    }
}
...
macro_rules! verify_with_overlapped_commitment_fold {
    (
        fold_publics: $fold_publics:expr,
        fold_commitments: $fold_commitments:expr,
        endpoint: |$folded_public:ident| $endpoint:expr $(,)?
    ) => {{
        #[cfg(feature = "parallel")]
        {
            let mut folded_commitments = None;
            let ($folded_public, subclaim) = rayon::in_place_scope(|scope| {
                scope.spawn(|_| {
                    folded_commitments = Some($fold_commitments);
                });
                let $folded_public = $fold_publics?;
                // Error returns preserve public-fold, commitment-fold, then endpoint precedence.
                // The transcript is still caller-owned state: on malformed commitments, parallel
                // builds may have run endpoint verification before returning the commitment error.
                let subclaim = $endpoint;
                Ok::<_, ProductionShaError<F>>(($folded_public, subclaim))
            })?;
            let folded_commitments =
                folded_commitments.expect("scoped folded commitment task always runs")?;
            let subclaim = subclaim?;
            ($folded_public, subclaim, folded_commitments)
        }
        #[cfg(not(feature = "parallel"))]
        {
            let $folded_public = $fold_publics?;
            let folded_commitments = $fold_commitments?;
            let subclaim = $endpoint?;
            ($folded_public, subclaim, folded_commitments)
        }
    }};
...
#[allow(type_alias_bounds)]
type ProductionShaMixedHyraxFoldAfterSumfold<C: AffineRepr, F> = (
    ProjectionFoldWitness<F>,
    ProjectedPublic<F>,
    F,
    InstanceFoldClaim<F>,
...
#[derive(Clone, Debug)]
pub struct ProductionShaMixedHyraxFoldedWitness<C, F>
where
    C: AffineRepr,
    F: PrimeField,
{
...
pub fn absorb_mixed_hyrax_production_sha_commitments<C>(
    transcript: &mut impl Transcript,
    label: &'static [u8],
    commitments: &[HyraxMixedCommitment<C>],
) where
    C: AffineRepr,
{
    transcript.absorb_slice(label);
    transcript.absorb_slice(b"production_sha_mixed_hyrax_commitments_v1");
    transcript.absorb_slice(&(commitments.len() as u64).to_le_bytes());
    for (instance_idx, commitment) in commitments.iter().enumerate() {
        transcript.absorb_slice(&(instance_idx as u64).to_le_bytes());
...
pub fn absorb_derived_mixed_hyrax_production_sha_commitments<C, F>(
    transcript: &mut impl Transcript,
    label: &'static [u8],
    commitments: &[HyraxMixedCommitment<C>],
    weights: &[F],
    ...
    F::Modulus: Transcribable,
{
    let mut field_buf = runtime_field_transcript_buf::<F>(field_cfg);
    transcript.absorb_slice(label);
    transcript.absorb_slice(b"derived_from_mixed_hyrax_fresh_v1");
    transcript.absorb_slice(&(commitments.len() as u64).to_le_bytes());
    transcript.absorb_slice(&(weights.len() as u64).to_le_bytes());
    transcript.absorb_random_field_slice(weights, &mut field_buf);
    ...
    F: PrimeField + FromPrimitiveWithConfig + ShaSmallFieldDecode + Send + Sync,
{
    ...
    cfg_iter!(witnesses)
        .map(|witness| {
            ...
            let (trace, public, witness_polys, native_view) =
            ...
            let sumfold_basis = tracing::info_span!(
                target: "zinc_protocol::production_sha",
                "prepare_sumfold_basis",
                side = "prepare",
                phase = "prepare_sumfold_basis",
            )
            .in_scope(|| {
                if let Some(native_view) = native_view.as_ref() {
                    prepare_sha_sumfold_basis_from_native_view(native_view, field_cfg)
                } else {
                    prepare_sha_sumfold_basis_production_fast(&trace, &public, field_cfg)
                }
            })?;
    ...
    P::IntPCS: FoldablePCS<F, Zt::Int, D>,
{
    ...
    let sumfold_bases = prepared_instances
        .iter()
        .map(|prepared| &prepared.sumfold_basis)
        .collect::<Vec<_>>();
    #[cfg(debug_assertions)]
...
#[allow(clippy::too_many_arguments)]
pub fn prove_prepared_linear_ideal_fold_mixed_hyrax<C, U, Zt, F, const D: usize>(
    ...
    LinearIdealFoldProveOutput<
        ...
        ProductionShaMixedHyraxProof<C, F>,
    >,
    LinearIdealFoldError<F>,
>
where
    ...
    IntScalarLane: HyraxLanes<C, Zt::Int, D>,
{
    ...
    let (folded, folded_public, row_claim, sumfold_output, folded_prover_data) =
        prove_mixed_hyrax_fold_after_sumfold_phase::<C, Zt, F, D>(
            &traces,
            &publics,
            sumfold_output,
            &r_ic_eq_weights,
    ...
    Ok(LinearIdealFoldProveOutput {
        ...
        proof: ProductionShaMixedHyraxProof {
            instance_commitments,
            ideal_check,
            sumfold_proof,
            resolver,
...
fn commit_production_sha_mixed_hyrax_instance<C, Zt, F, const D: usize>(
    pcs_params: &PCSParams<AllHyraxPCSTypes<C>, Zt, F, D>,
    witness_polys: &ProductionShaWitnessPolys<Zt, D>,
) -> Result<(HyraxMixedProverData<C>, HyraxMixedCommitment<C>), ProductionShaError<F>>
where
    Zt: ZincTypes<D>,
    F: PrimeField,
    ...
    IntScalarLane: HyraxLanes<C, Zt::Int, D>,
{
    ...
    .in_scope(|| {
        <HyraxPCS<C, BinaryLanes> as PCS<F, BinaryPoly<D>, D>>::precompute_ck(&pcs_params.binary);
...
#[tracing::instrument(
    ...
    level = "info",
    skip_all,
    fields(side = "prove", phase = "sumfold_linear_accumulator", instances = sumfold_bases.len())
)]
...
{
    if is_production_sha_booleanity_sources(booleanity_sources)
        && sumfold_bases
            .iter()
...
fn prove_mixed_hyrax_fold_after_sumfold_phase<C, Zt, F, const D: usize>(
    ...
    _booleanity_weights: &[F],
    _booleanity_sources: &[ShaBooleanitySource],
    instance_prover_data: &[HyraxMixedProverData<C>],
    field_cfg: &F::Config,
) -> Result<ProductionShaMixedHyraxFoldAfterSumfold<C, F>, ProductionShaError<F>>
where
    Zt: ZincTypes<D>,
    F: InnerTransparentField + DelayedFieldProductSum + ShaBinaryFoldField + Send + Sync + 'static,
    F: HyraxFieldBridge<C>,
    ...
    F: InnerTransparentField + DelayedFieldProductSum + Send + Sync + 'static,
{
    ...
    let values = cfg_iter!(row_weights)
        ...
        .map(|(row, row_weight)| {
            ...
            let bool_sum = if booleanity_sources.len() == PRODUCTION_SHA_BOOLEANITY_SOURCE_COUNT {
                let mut bool_error = None;
                let bool_terms: [F; PRODUCTION_SHA_BOOLEANITY_SOURCE_COUNT] =
                    std::array::from_fn(|idx| {
                        if bool_error.is_some() {
                            return F::zero_with_cfg(field_cfg);
                        }
                        match booleanity_source_value_at_fast(
                            trace,
                            row,
                            &booleanity_sources[idx],
                            field_cfg,
                        ) {
                            Ok(d) => d.clone() * (d - &one),
                            Err(err) => {
                ...
                for source in booleanity_sources.iter() {
                    let d = booleanity_source_value_at_fast(trace, row, source, field_cfg)?;
                    bool_terms.push(d.clone() * (d - &one));
                }
                F::delayed_sum_of_products(
                    booleanity_weights,
                    &bool_terms,
...
#[allow(clippy::too_many_arguments)]
fn prove_mixed_hyrax_pcs_opening_phase<C, Zt, F, const D: usize>(
    transcript: &mut impl Transcript,
    folded_trace: &ProjectedTrace<F>,
    instance_commitments: &[HyraxMixedCommitment<C>],
    fold_weights: &[F],
    folded_prover_data: &HyraxMixedProverData<C>,
    r_0: &[F],
    r_0_eq_weights: &[F],
    pcs_params: &PCSParams<AllHyraxPCSTypes<C>, Zt, F, D>,
    field_cfg: &F::Config,
    ...
    P::IntPCS: FoldablePCS<F, Zt::Int, D>,
{
    ...
    // Parallel error paths can advance the caller-owned transcript before a commitment-fold error
    // is returned; accepted proofs and returned error precedence match the serial path.
    let (folded_public, subclaim, folded_commitments) = verify_with_overlapped_commitment_fold!(
        fold_publics: verify_fold_publics_phase(
            &publics,
            sumfold_output.eq_instance_weights(),
            field_cfg,
        ),
        fold_commitments: verify_fold_commitments_phase::<P, Zt, F, D>(
            &proof.instance_commitments,
            sumfold_output.eq_instance_weights(),
            field_cfg,
        ),
        endpoint: |folded_public| verify_endpoint_multipoint_phase(
            transcript,
            proof,
            &folded_public,
            &row_output,
    ...
    IntScalarLane: HyraxLanes<C, Zt::Int, D>,
{
    ...
    let projected = cfg_iter!(instances, PRODUCTION_SHA_PUBLIC_PROJECTION_MIN_PAR_LEN)
        .map(|instance| {
            U::project_production_sha_public(&vs.shape, &instance.public_trace, field_cfg)
        })
        .collect::<Vec<_>>();
...
#[allow(clippy::too_many_arguments)]
fn verify_mixed_hyrax_pcs_phase<C, Zt, F, const D: usize>(
    transcript: &mut impl Transcript,
...
fn build_sha_endpoint_scalarization_powers<F>(a: &F, field_cfg: &F::Config) -> [F; SHA_WORD_BITS]
where
    F: PrimeField,
{
...
fn scalarize_sha_endpoint_bits_with_powers<F>(
    bits: &[F; SHA_WORD_BITS],
    powers: &[F; SHA_WORD_BITS],
    field_cfg: &F::Config,
) -> F
where
    F: PrimeField,
    ...
    F: DelayedFieldProductSum,
{
    ...
    let a_powers = build_sha_endpoint_scalarization_powers(a, field_cfg);
    ...
    for (col, shift) in word_sources {
        ...
        let scalarized = scalarize_sha_endpoint_bits_with_powers(&bits, &a_powers, field_cfg);
...
fn absorb_derived_mixed_hyrax_pcs_commitment<C, F>(
    transcript: &mut impl Transcript,
    label: &'static [u8],
    commitments: &[HyraxMixedCommitment<C>],
    weights: &[F],
    ...
    F::Modulus: Transcribable,
{
    let mut field_buf = runtime_field_transcript_buf::<F>(field_cfg);
    transcript.absorb_slice(label);
    transcript.absorb_slice(b"derived_mixed_hyrax_pcs_commitment_v1");
    transcript.absorb_slice(&(commitments.len() as u64).to_le_bytes());
    transcript.absorb_slice(&(weights.len() as u64).to_le_bytes());
    transcript.absorb_random_field_slice(weights, &mut field_buf);
    for (idx, commitment) in commitments.iter().enumerate() {
...
#[allow(clippy::too_many_arguments)]
fn prove_production_sha_mixed_hyrax_pcs_opening<C, Zt, F, const D: usize>(
    pcs_params: &PCSParams<AllHyraxPCSTypes<C>, Zt, F, D>,
    instance_commitments: &[HyraxMixedCommitment<C>],
    fold_weights: &[F],
    folded_trace: &ProjectedTrace<F>,
    ...
    IntScalarLane: HyraxLanes<C, Zt::Int, D>,
{
    ...
    absorb_derived_mixed_hyrax_pcs_commitment(
        &mut transcript.fs_transcript,
        b"production_sha_pcs_mixed_binary_int",
        instance_commitments,
        fold_weights,
        field_cfg,
    );
    ...
    HyraxPCS::<C, BinaryLanes>::prove_open_two_field_lane_groups_single_row::<F, true>(
        &mut transcript,
        &pcs_params.binary,
        &binary_field_lanes,
        &folded_prover_data.binary,
...
#[allow(clippy::too_many_arguments)]
fn verify_production_sha_mixed_hyrax_pcs_opening<C, Zt, F, const D: usize>(
    pcs_params: &PCSVerifierParams<AllHyraxPCSTypes<C>, Zt, F, D>,
    instance_commitments: &[HyraxMixedCommitment<C>],
    fold_weights: &[F],
    r_0: &[F],
    ...
    IntScalarLane: HyraxLanes<C, Zt::Int, D>,
{
    ...
    let first_commitment =
        instance_commitments
            .first()
            .ok_or_else(|| ProductionShaError::LengthMismatch {
                label: "mixed Hyrax commitments",
                got: 0,
                expected: 1,
            })?;
    validate_production_sha_batch_sizes::<F>(
    ...
    let int_commitments = instance_commitments
        ...
        .map(|commitment| &commitment.int)
        .collect::<Vec<_>>();
    HyraxPCS::<C, BinaryLanes>::verify_open_two_field_lane_groups_single_row_folded::<
        F,
        BinaryPoly<D>,
        BinaryLanes,
        Zt::Int,
    ...
    IntScalarLane: HyraxLanes<C, Zt::Int, D>,
{
    ...
    let first_commitment =
        instance_commitments
            .first()
            .ok_or_else(|| ProductionShaError::LengthMismatch {
                label: "packed Hyrax commitments",
                got: 0,
                expected: 1,
            })?;
    if <HyraxPCS<C, ScalarFieldLane> as PCS<F, C::ScalarField, D>>::batch_size(first_commitment)
        != 1
    {
        return Err(ProductionShaError::UnsupportedProductionShaPcsShape(
    ...
    let commitment_refs = instance_commitments.iter().collect::<Vec<_>>();
    ...
    P::IntPCS: FoldablePCS<F, Zt::Int, D>,
{
    ...
    let int = commitments
        ...
        .map(|commitment| &commitment.int)
        .collect::<Vec<_>>();
    let (binary, (arbitrary, int)) = cfg_join!(
        P::BinaryPCS::fold_commitment_refs(&binary, theta, field_cfg),
        P::ArbitraryPCS::fold_commitment_refs(&arbitrary, theta, field_cfg),
        P::IntPCS::fold_commitment_refs(&int, theta, field_cfg),
    );
    Ok(PCSCommitments {
        binary: binary?,
        arbitrary: arbitrary?,
...
pub fn fold_mixed_hyrax_prover_data<C, Zt, F, const D: usize>(
    prover_data: &[HyraxMixedProverData<C>],
    theta: &[F],
    field_cfg: &F::Config,
) -> Result<HyraxMixedProverData<C>, ProductionShaError<F>>
where
    Zt: ZincTypes<D>,
    F: PrimeField,
    F: HyraxFieldBridge<C>,
    ...
    IntScalarLane: HyraxLanes<C, Zt::Int, D>,
{
    if prover_data.len() != theta.len() {
        return Err(ProductionShaError::LengthMismatch {
            label: "mixed hyrax prover_data/theta",
            got: prover_data.len(),
            expected: theta.len(),
        });
    }
    ...
    let int = prover_data
        ...
        .map(|data| data.int.clone())
        .collect::<Vec<_>>();
    Ok(HyraxMixedProverData {
        binary: <HyraxPCS<C, BinaryLanes> as FoldablePCS<F, BinaryPoly<D>, D>>::fold_prover_data(
            &binary, theta, field_cfg,
        )?,
        int: <HyraxPCS<C, IntScalarLane> as FoldablePCS<F, Zt::Int, D>>::fold_prover_data(
            &int, theta, field_cfg,
        )?,
    })
}
    ...
    F: InnerTransparentField + DelayedFieldProductSum + Send + Sync + 'static,
{
    ...
    Ok(MultiDegreeSumcheckGroup::new(
        ...
        Box::new(move |values: &[F]| {
            ...
            let booleanity_term = |source: &ShaBooleanitySource| {
                let d = match *source {
            ...
            let bool_sum = if booleanity_sources.len() == PRODUCTION_SHA_BOOLEANITY_SOURCE_COUNT {
                let bool_terms: [F; PRODUCTION_SHA_BOOLEANITY_SOURCE_COUNT] =
                    std::array::from_fn(|idx| booleanity_term(&booleanity_sources[idx]));
                F::delayed_sum_of_products(&booleanity_weights, &bool_terms, zero.clone())
            } else {
                let bool_terms = booleanity_sources
                    .iter()
                    .map(booleanity_term)
                    .collect::<Vec<_>>();
                F::delayed_sum_of_products(&booleanity_weights, &bool_terms, zero.clone())
    ...
    let sources = cfg_into_iter!(0..word_sources.len())
        .map(|idx| {
            ...
            Ok(ShaSourceEndpointEval {
                col,
                shift,
                scalarized: scalarize_sha_endpoint_bits_with_powers(&bits, &a_powers, field_cfg),
                bits,
            })
        })
        .collect::<Result<Vec<_>, ProductionShaError<F>>>()?;
    ...
    F: DelayedFieldProductSum + Send + Sync,
{
    ...
    let public_pain = endpoint_public_const_poly_with_row_weights(
        folded_public,
        ShaPublicCol::PAIn,
        0,
        row_weights,
    ...
    let public_pein = endpoint_public_const_poly_with_row_weights(
        folded_public,
        ShaPublicCol::PEIn,
        0,
        row_weights,
    ...
    let r0 = mul_endpoint_poly_by_sparse_positions(&a, &[10, 19, 30], field_cfg)
        - &sigma0
        - &scale_endpoint_poly(&ov_sigma0, &two);
    let r1 = mul_endpoint_poly_by_sparse_positions(&e, &[7, 21, 26], field_cfg)
        - &sigma1
        - &scale_endpoint_poly(&ov_sigma1, &two);
    let r2 = w.rot_c(25) + &w.rot_c(14) + &w.shift_r_c(3)
        - &small_sigma0
        - &scale_endpoint_poly(&ov_small_sigma0, &two);
    let r3 = w.rot_c(15) + &w.rot_c(13) + &w.shift_r_c(10)
        - &small_sigma1
        - &scale_endpoint_poly(&ov_small_sigma1, &two);

    let mu_low_coeff = endpoint_pow_two(32, field_cfg);
    let mu_high_w_coeff = endpoint_pow_two(34, field_cfg);
    let mu_high_3_bit_coeff = endpoint_pow_two(35, field_cfg);
    let mu_high_1_bit_coeff = endpoint_pow_two(33, field_cfg);
    let mu_w =
        endpoint_mu_contribution_from_packed(&mu_packed, 0, 2, &mu_low_coeff, &mu_high_w_coeff);
    let mu_a =
        endpoint_mu_contribution_from_packed(&mu_packed, 2, 5, &mu_low_coeff, &mu_high_3_bit_coeff);
    let mu_e =
        endpoint_mu_contribution_from_packed(&mu_packed, 5, 8, &mu_low_coeff, &mu_high_3_bit_coeff);
    let mu_ff_a =
        endpoint_mu_contribution_from_packed(&mu_packed, 8, 9, &mu_low_coeff, &mu_high_1_bit_coeff);
    let mu_ff_e = endpoint_mu_contribution_from_packed(
        &mu_packed,
        9,
        10,
        &mu_low_coeff,
    ...
    let r9 = a_shift4 - &a - &public_pain + &mu_ff_a + &comp_ff_a;
    let r10 = e_shift4 - &e - &public_pein + &mu_ff_e + &comp_ff_e;
    ...
    let r17 = mu_packed.shift_r_c(10);
...
fn endpoint_mu_contribution_from_packed<F>(
    packed_mu: &DynamicPolynomialF<F>,
    low: usize,
    high: usize,
    low_coeff: &F,
    high_coeff: &F,
...
where
    F: PrimeField,
{
    let packed = packed_mu.shift_r_c(low as u32);
    let tail = packed_mu.shift_r_c(high as u32);
    scale_endpoint_poly(&packed, low_coeff) - &scale_endpoint_poly(&tail, high_coeff)
}
...
fn mul_endpoint_poly_by_sparse_positions<F>(
    poly: &DynamicPolynomialF<F>,
    positions: &[usize],
    field_cfg: &F::Config,
) -> DynamicPolynomialF<F>
where
    F: PrimeField,
{
    if poly.is_zero() || positions.is_empty() {
    ...
    let mut coeffs = vec![F::zero_with_cfg(field_cfg); poly.coeffs.len() + max_pos];
    for (pos_idx, &pos) in positions.iter().enumerate() {
        debug_assert!(pos < SHA_WORD_BITS);
        debug_assert!(positions[..pos_idx].iter().all(|previous| *previous != pos));
        for (idx, coeff) in poly.coeffs.iter().enumerate() {
            coeffs[idx + pos] += coeff;
        }
    }
    DynamicPolynomialF::new_trimmed(coeffs)
}
...
mod tests {
    ...
    fn mle_bit_slices_from_native_word_bits<F, const COLS: usize>(
    ...
    fn project_binary_sources_to_native_word_bits<F>(
        cols: &[&DenseMultilinearExtension<BinaryPoly<TEST_DEGREE_PLUS_ONE>>],
        field_cfg: &<F as PrimeField>::Config,
        ...
        F: PrimeField,
    {
        ...
        Ok((mle_table_from_columns(flat_columns), packed_columns))
    }
    ...
    fn sha_int_to_i64<G: PrimeField>(value: &ShaInt) -> Result<i64, ProductionShaError<G>> {
        use crypto_primitives::IntRing;
        let (abs, is_negative) = if value.is_negative() {
            (
                value
    ...
    fn pack_binary_source_bits<F: PrimeField>(
        col: &DenseMultilinearExtension<BinaryPoly<TEST_DEGREE_PLUS_ONE>>,
    ...
    fn project_int_source_with_values<F: TestShaField>(
        col: &DenseMultilinearExtension<ShaInt>,
        field_cfg: &<F as PrimeField>::Config,
        ...
        for (row, value) in col.evaluations.iter().take(SHA_ROW_COUNT).enumerate() {
            native[row] = sha_int_to_i64::<F>(value)?;
            projected.push(F::from_with_cfg(native[row], field_cfg));
        }
        Ok((projected, native))
    }

    fn packed_word_row_value(bits: &[u128; SHA_WORD_BITS], row: usize) -> i64 {
        let mut value = 0i64;
    ...
    fn production_sha_selector_expected_i64(selector: ShaPublicCol, row: usize) -> i64 {
        match selector {
    ...
    fn native_public_values_from_bits(
        ...
        for row in 0..SHA_ROW_COUNT {
            columns[ShaPublicCol::K.index()][row] = if (3..67).contains(&row) {
                i64::from(SHA256_ROUND_CONSTANTS[row - 3])
            } else {
                0
            };
            columns[ShaPublicCol::PAIn.index()][row] = packed_word_row_value(pa_a, row);
            columns[ShaPublicCol::PEIn.index()][row] = packed_word_row_value(pa_e, row);
            columns[ShaPublicCol::PAOut.index()][row] = packed_word_row_value(pa_a, row);
            columns[ShaPublicCol::PEOut.index()][row] = packed_word_row_value(pa_e, row);
            columns[ShaPublicCol::Message.index()][row] = packed_word_row_value(message, row);
        }
        for selector in [
            ShaPublicCol::SInit,
            ShaPublicCol::SMsg,
            ...
            for row in 0..SHA_ROW_COUNT {
                columns[selector.index()][row] =
                    production_sha_selector_expected_i64(selector, row);
            }
        }
        columns
    }

    fn mle_table_from_native_i64_columns<F, const COLS: usize>(
    ...
    fn scalarized_from_native_word_bits<F>(
        ...
        F: PrimeField + FromPrimitiveWithConfig,
    {
        let columns = word_bits
            ...
            .map(|bits| {
                (0..SHA_ROW_COUNT)
                    .map(|row| F::from_with_cfg(packed_word_row_value(bits, row), field_cfg))
                    .collect()
            })
            .collect();
        mle_table_from_columns(columns)
        ...
        for Sha256CompressionSliceUair<ShaInt>
    {
        ...
        > {
            ...
            let word_cols = word_sources
                .iter()
                .map(|&col| sha_binary_col(public_trace, witness_trace, col))
                .collect::<Result<Vec<_>, _>>()?;
            let (bit_slices, word_bit_vec) =
                project_binary_sources_to_native_word_bits::<F>(&word_cols, field_cfg)?;
            ...
            let scalarized = scalarized_from_native_word_bits::<F>(&word_bits, field_cfg);
            let pa_a_bits = pack_binary_source_bits::<F>(sha_binary_col(
                public_trace,
                witness_trace,
                sha256_cols::PA_A,
            )?)?;
            let pa_e_bits = pack_binary_source_bits::<F>(sha_binary_col(
                public_trace,
                witness_trace,
                sha256_cols::PA_E,
            )?)?;
            let message_bits = pack_binary_source_bits::<F>(sha_binary_col(
                public_trace,
                witness_trace,
                sha256_cols::PA_M,
            )?)?;
            let public_word_bits = [pa_a_bits, pa_e_bits, pa_a_bits, pa_e_bits, message_bits];
            let public_bit_slices = mle_bit_slices_from_native_word_bits::<
                F,
                { ShaPublicWordCol::COUNT },
            >(&public_word_bits, field_cfg);
            let public_values =
                native_public_values_from_bits(&pa_a_bits, &pa_e_bits, &message_bits);
            let public_columns = mle_table_from_native_i64_columns::<F, { ShaPublicCol::COUNT }>(
                &public_values,
                field_cfg,
            );

            let int_projection = int_sources
                .iter()
                .map(|&col| {
                    project_int_source_with_values(
                        sha_int_col(public_trace, witness_trace, col)?,
                        field_cfg,
                    )
                })
            ...
            let int_values: [[i64; SHA_ROW_COUNT]; ShaIntCol::COUNT] = int_value_vec
                .try_into()
                .map_err(|values: Vec<[i64; SHA_ROW_COUNT]>| {
                    ProductionShaError::LengthMismatch {
            ...
            let trace = ProjectedTrace {
                bit_slices,
                scalarized,
                int_columns: mle_table_from_columns(int_columns),
                public_columns: public_columns.clone(),
            };
            let public = ProjectedPublic {
                columns: public_columns,
            ...
            >,
    {
        ...
        let (shared_ck, shared_vk) = hyrax_key_pair::<C, BinaryLanes>(width, 0);
        let (arbitrary_ck, arbitrary_vk) = hyrax_key_pair::<C, DensePolyScalarLanes>(width, 1_000);

        (
            PCSParams::<AllHyraxPCSTypes<C>, TestShaZincTypes, F, TEST_DEGREE_PLUS_ONE> {
                binary: shared_ck.clone(),
                arbitrary: arbitrary_ck,
                int: shared_ck,
            },
            PCSVerifierParams::<AllHyraxPCSTypes<C>, TestShaZincTypes, F, TEST_DEGREE_PLUS_ONE> {
                binary: shared_vk.clone(),
    ...
    -> ProductionShaWitnessPolys<TestShaZincTypes, TEST_DEGREE_PLUS_ONE> {
        ...
        .unwrap();
        let (_, _, witness_polys) = <U as ProductionShaProjectionAdapter<
            TestShaZincTypes,
            F,
            TEST_DEGREE_PLUS_ONE,
        >>::project_production_sha_witness(
            &shape, &public_trace, &witness_trace, &field_cfg
        )
        .unwrap();
        witness_polys
    }
    ...
    #[test]
    fn production_sha_native_view_matches_projection_and_reference_basis() {
        type TestF = ArkF;
        type U = Sha256CompressionSliceUair<ShaInt>;
        ...
        let (witnesses, _final_state) =
            synthesize_sha256_chain_witnesses::<ShaInt, 8>(SHA256_INITIAL_STATE, message_blocks)
                .expect("SHA-256 UAIR witnesses synthesize");
        let shape = UairShape::<U>::new(SHA_ROW_VARS);
        let witness = &witnesses[0];
        let public_trace = public_uair_trace_view::<ShaInt, ShaInt, TestF, TEST_DEGREE_PLUS_ONE>(
            &witness.trace,
            &shape.signature,
        )
        .unwrap();
        let witness_trace = witness_uair_trace_view::<ShaInt, ShaInt, TestF, TEST_DEGREE_PLUS_ONE>(
            &witness.trace,
            &shape.signature,
        )
        .unwrap();
        let (trace, public, _witness_polys, native_view) =
            <U as ProductionShaProjectionAdapter<
                TestShaZincTypes,
                TestF,
                TEST_DEGREE_PLUS_ONE,
            >>::project_production_sha_witness_with_native_view(
                &shape,
                &public_trace,
                &witness_trace,
                &field_cfg,
            ...
            .unwrap();
        let native_view = native_view.expect("production adapter should provide native view");
        let one = TestF::one_with_cfg(&field_cfg);

        for col in ShaWordCol::ALL {
            for bit in 0..SHA_WORD_BITS {
                let table_idx = bit_slice_index(col.index(), bit, SHA_WORD_BITS);
                for row in 0..SHA_ROW_COUNT {
                    let projected = trace.bit_slices[table_idx].evaluations[row] == one;
                    let native = ((native_view.word_bits[col.index()][bit] >> row) & 1) == 1;
                    assert_eq!(native, projected, "word {col:?} bit {bit} row {row}");
                }
            }
        }
        for col in ShaIntCol::ALL {
            for row in 0..SHA_ROW_COUNT {
                let encoded =
                    TestF::from_with_cfg(native_view.int_values[col.index()][row], &field_cfg);
                assert_eq!(
                    encoded,
                    trace.int_columns[col.index()].evaluations[row],
                    "int {col:?} row {row}"
                );
            }
        }
        let public_bits = public
            .bit_slices
            .as_ref()
            .expect("production public projection includes word bits");
        for col in ShaPublicWordCol::ALL {
            for bit in 0..SHA_WORD_BITS {
                let table_idx = bit_slice_index(col.index(), bit, SHA_WORD_BITS);
                for row in 0..SHA_ROW_COUNT {
                    let projected = public_bits[table_idx].evaluations[row] == one;
                    let native = ((native_view.public_word_bits[col.index()][bit] >> row) & 1) == 1;
                    assert_eq!(native, projected, "public word {col:?} bit {bit} row {row}");
                }
            }
        }
        for col in ShaPublicCol::ALL {
            for row in 0..SHA_ROW_COUNT {
                let encoded =
                    TestF::from_with_cfg(native_view.public_values[col.index()][row], &field_cfg);
                assert_eq!(
                    encoded,
                    public.columns[col.index()].evaluations[row],
                    "public {col:?} row {row}"
                );
            }
        }
        ...
        let direct =
            prepare_sha_sumfold_basis_from_native_view::<TestF>(&native_view, &field_cfg).unwrap();
        let reference = prepare_sha_sumfold_basis(&trace, &public, &field_cfg).unwrap();
        assert!(direct.residual_basis.rows.is_empty());
        assert_eq!(direct.small_residual_basis, reference.small_residual_basis);
        assert_eq!(direct.booleanity_basis, reference.booleanity_basis);
    }
    ...
    fn mixed_hyrax_linear_ideal_fold_proves_and_verifies_eight_sha_instances() {
        ...
        let mut verifier_transcript = Blake3Transcript::new();
        let verified = verify_linear_ideal_fold_mixed_hyrax::<
            C,
            U,
            TestShaZincTypes,
            F,
    ...
    #[test]
    fn fold_projected_publics_fast_path_matches_naive_reference() {
        let field_cfg = cfg();
        let mle = |values: &[u64]| DenseMultilinearExtension {
            evaluations: values
                .iter()
                .map(|value| F::from_with_cfg(*value, &field_cfg))
                .collect(),
        ...
        let publics = vec![
            ProjectedPublic {
                columns: vec![mle(&[0, 1, 2, 1]), mle(&[3, 0, 1, 4])],
                bit_slices: Some(vec![mle(&[1, 0, 1, 0]), mle(&[0, 1, 0, 1])]),
            },
            ProjectedPublic {
                columns: vec![mle(&[1, 0, 5, 2]), mle(&[0, 1, 7, 1])],
                bit_slices: Some(vec![mle(&[0, 1, 1, 0]), mle(&[1, 0, 0, 1])]),
            },
        ];
        let theta = vec![f(11), f(13)];

        let naive_fold = |tables: &[&MleTable<F>]| {
            assert!(!tables.is_empty());
            (0..tables[0].len())
                .map(|col_idx| {
                    let mut evaluations =
                        vec![F::zero_with_cfg(&field_cfg); tables[0][col_idx].evaluations.len()];
                    for (table, weight) in tables.iter().zip(theta.iter()) {
                        for (out, value) in evaluations
                            .iter_mut()
                            .zip(table[col_idx].evaluations.iter())
                    ...
                    DenseMultilinearExtension {
                        evaluations,
                        num_vars: tables[0][col_idx].num_vars,
                    }
                })
                .collect::<MleTable<F>>()
        };

        let column_tables = [&publics[0].columns, &publics[1].columns];
        let bit_slice_tables = [
            publics[0].bit_slices.as_ref().unwrap(),
            publics[1].bit_slices.as_ref().unwrap(),
        ];
        let expected = ProjectedPublic {
            columns: naive_fold(&column_tables),
            bit_slices: Some(naive_fold(&bit_slice_tables)),
        };
        let folded = fold_projected_publics(&publics, &theta, &field_cfg).unwrap();
        assert_eq!(folded, expected);
    }

    #[test]
    fn endpoint_scalarization_with_powers_matches_naive_reference() {
        let field_cfg = cfg();
        let a = f(7);
        let bits = std::array::from_fn(|idx| f(idx as u64 + 3));
        let powers = build_sha_endpoint_scalarization_powers(&a, &field_cfg);
        let mut power = F::one_with_cfg(&field_cfg);
        let mut expected = F::zero_with_cfg(&field_cfg);
        for bit in bits.iter() {
            expected += bit.clone() * &power;
            power = power * &a;
        }

        assert_eq!(
            scalarize_sha_endpoint_bits_with_powers(&bits, &powers, &field_cfg),
            expected
        );
    }

    #[test]
    fn sparse_position_multiplication_matches_sparse_polynomial_product() {
        let field_cfg = cfg();
        let poly = DynamicPolynomialF::new_trimmed(vec![f(3), f(5), f(0), f(11), f(17)]);
        assert!(
            mul_endpoint_poly_by_sparse_positions::<F>(
                &DynamicPolynomialF::ZERO,
                &[10],
                &field_cfg
            )
        ...
        assert!(mul_endpoint_poly_by_sparse_positions(&poly, &[], &field_cfg).is_zero());

        for positions in [&[10usize, 19, 30][..], &[7usize, 21, 26][..]] {
            let sparse = sparse_endpoint_poly(positions, &field_cfg);
            let expected = &poly * &sparse;
            let actual = mul_endpoint_poly_by_sparse_positions(&poly, positions, &field_cfg);
