//! **RS-equivalent variant of `f2_sha256.rs`**.
//!
//! Identical to [`f2_sha256`](./f2_sha256.rs) except for the column
//! opening count: this file uses **`BENCH_NUM_OPENINGS = 200`**
//! (proximity-bound for a Reed-Solomon / MDS code) instead of the
//! RAA-calibrated **`987`** used by its sibling. The Criterion group
//! names are also suffixed with ` RS` (e.g. `Zinc+ F_2 SHA-256 RS`,
//! `Zinc+ F_2 SHA-256 RS Steps`, `Zinc+ F_2 SHA-256 RS Micro`) so the
//! two bench targets store their baselines under disjoint paths and
//! can be run side-by-side without overwriting each other.
//!
//! Caveat: this bench keeps the existing `RaaF2Code` encoder. The
//! `200` value reflects only the *number of column openings* a
//! Reed-Solomon code would let us send; the actual encoding cost and
//! proximity proof are NOT switched. Use it to estimate the
//! prover/verifier wall-time and proof-size impact of cutting
//! openings ~5×, not to model an end-to-end RS protocol swap (see
//! [`documentation/f2-prove-optimizations.md`](../../documentation/f2-prove-optimizations.md)
//! § "RAA → RS swap" for why the full swap is non-trivial).
//!
//! ## What the original bench measures
//!
...
//!
//! 1. `Zinc+ F_2 SHA-256 RS` — end-to-end (WitnessGen / Prove / Verify).
//! 2. `Zinc+ F_2 SHA-256 RS Steps` — top-level prover and verifier step
//!    breakdown (Commit / UAIR / Open).
//! 3. `Zinc+ F_2 SHA-256 RS Micro` — fine-grained timings *inside* the
...
/// Number of column openings the prover sends to the verifier. The
/// canonical value for RAA at REP=4 is
/// `recommended_num_column_openings(REP) = 987`, calibrated for the
/// RAA proximity gap. We override here to **200** to estimate the
/// proof size and prover/verifier wall-time *as if* the protocol used
/// a Reed-Solomon code with the corresponding MDS proximity bound.
/// Switching back to the soundness-calibrated value is one symbol
/// change away.
const BENCH_NUM_OPENINGS: usize = 200;
...
fn f2_full_proof_parts(proof: &F2FullProof<D>) -> Vec<(&'static str, Vec<u8>)> {
    ...
    // The un-lifted open carries its claim / b-vector / combined-row as
    // `GF128Poly<D>` (D = WORD_BITS GF(2^128) coefficients = the bit-slice evals)
    // rather than a wide `BinaryF2Poly`. Each poly serialises as `D · ALPHA_BYTES`.
...
fn bench_micro_prover_uair(
    ...
    // ---- 0b') Commit-Encode-RS: same shape, but with bolt-rs's
    //          (ligerito-derived) FFT-based Reed-Solomon encoder
    //          instead of RaaF2Code.
    //
    // Goal: compare raw encode throughput RS vs RAA at the F_2 commit
    // shape. NOT a drop-in replacement for the protocol (see
    // documentation/f2-prove-optimizations.md § "RAA → RS swap" for the
    // open-phase / F2LinearOpener / soundness barriers). We only measure
    // the encode-step cost here.
    //
    // Cell mapping: each paired `BinaryPoly<64>` cell holds two
    // `BinaryPoly<32>` halves (low/high u32). For RS we treat each half
    // as one `BinaryElem32` (= GF(2^32) under bolt-rs's GHASH-style
    // pentanomial), so one paired matrix becomes two unpaired RS
    // matrices over `BinaryElem32`. Same total bytes encoded.
    //
    // Rate: 1/2 (matches bolt-rs / ligerito's natural rate). NOT the
    // same rate as `Commit-Encode` (which uses RaaF2Code at REP=4,
    // i.e. rate 1/4) — the comparison is intentional: RS at its
    // natural rate vs RAA at its calibrated rate. Rate 1/2 means
    // block_length = 2 × row_len, so the RS encoder does roughly half
    // the FFT work of the rate-1/4 alternative (and produces a
    // proportionally smaller codeword).
    //
    // Encoder setup (twiddles + π polynomials) is built once outside
    // `bench.iter`; only the per-row `encode` work is measured.
    ...
    group.bench_function(BenchmarkId::new("Commit-Encode-RS", id), |bench| {
        ...
        // Precompute the RS encoder once. ReedSolomonEncoding::new
        // builds twiddles + π polynomials for the given (message,
        // block) lengths — that setup is NOT part of the per-encode
        // cost we want to measure.
        let rs = ReedSolomonEncoding::<BinaryElem32>::new(row_len, rs_block_length);

        // Convert paired BinaryPoly<64> matrices into 2× unpaired
        // BinaryElem32 matrices (low/high u32 halves). Stored as a
        // flat Vec<(matrix_idx, row_idx, row_data)> so the bench loop
        // can rayon-flatten the encode work in one go.
        let inputs: Vec<Vec<BinaryElem32>> = paired
            .iter()
            .flat_map(|poly| {
                let mut out: Vec<Vec<BinaryElem32>> = Vec::with_capacity(2 * num_rows);
                for half in 0..2usize {
                    for r in 0..num_rows {
                        let row: Vec<BinaryElem32> = (0..row_len)
                            .map(|j| {
        ...
        bench.iter(|| {
            let encoded: Vec<Vec<BinaryElem32>> =
                inputs.par_iter().map(|row| rs.encode(row)).collect();
            black_box(encoded);
        });
    });
...
fn e2e_benches(c: &mut Criterion) {
    let mut group = c.benchmark_group("Zinc+ F_2 SHA-256 RS");
...
fn step_benches(c: &mut Criterion) {
    let mut group = c.benchmark_group("Zinc+ F_2 SHA-256 RS Steps");
...
fn micro_benches(c: &mut Criterion) {
    let mut group = c.benchmark_group("Zinc+ F_2 SHA-256 RS Micro");
