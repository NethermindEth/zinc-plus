...
**Batched** Zip+ with BPoly<15> evaluations, num_rows=1, and depth=2.

This script runs the "Batched PCS Pipeline Suite BPoly15 1row" criterion benchmarks
...
Usage:
    python3 scripts/bench_batched_pcs_pipeline_bpoly15.py
    python3 scripts/bench_batched_pcs_pipeline_bpoly15.py --no-run --input bench_output.txt
    python3 scripts/bench_batched_pcs_pipeline_bpoly15.py --output batched_pcs_table_bpoly15.tex
"""
...
BENCH_FILTER = "Batched PCS Pipeline Suite BPoly15 1row/.* poly_size=2\\^(9|10|11) "
CARGO_BENCH_CMD = [
    "cargo", "bench",
...
def generate_latex_table(
    ...
    lines = [
        r"\begin{table}[ht]",
        r"\centering",
        r"\caption{Batched PCS pipeline benchmark (batch=" + str(BATCH_SIZE) + r") for BPoly\textlangle 15\textrangle{} with IPRS num\_rows=1, depth=2 (parallel+asm+simd).}",
        r"\label{tab:batched-pcs-pipeline-suite-bpoly15-1row-d2}",
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Batched PCS pipeline (Encode/Merkle/Commit/Test/Verify) benchmarks for BPoly<15> with num_rows=1, depth=2 and produce a LaTeX table."
    )
    parser.add_argument(
        "--no-run",
        action="store_true",
