...
**Batched** Zip+ with i32 evaluations and batch_size=3.

This script runs the "Batched PCS Pipeline Suite i32 1row batch3" criterion
benchmarks and collects the results into a LaTeX table.  The benchmarks batch 3
...
Usage:
    python3 scripts/bench_batched_pcs_pipeline_i32_batch3.py
    python3 scripts/bench_batched_pcs_pipeline_i32_batch3.py --no-run --input bench_output.txt
    python3 scripts/bench_batched_pcs_pipeline_i32_batch3.py --output batched_pcs_table_i32_batch3.tex
"""
...
BENCH_FILTER = "Batched PCS Pipeline Suite i32 1row batch3/.* poly_size=2\\^(10|11|12|13) "
CARGO_BENCH_CMD = [
    "cargo", "bench",
...
# Number of polynomials in the batch (must match BATCH_SIZE_3 in benchmarks).
BATCH_SIZE = 3
...
def parse_criterion_output(
    ...
    """
    Parse criterion benchmark output for the batched PCS pipeline suite (i32, batch3).
...
def generate_latex_table(
    ...
    lines = [
        ...
        r"\label{tab:batched-pcs-pipeline-suite-i32-1row-r4-batch3}",
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Batched PCS pipeline (Encode/Merkle/Commit/Test/Verify) benchmarks for i32 (batch=3) and produce a LaTeX table."
    )
    parser.add_argument(
        "--no-run",
        action="store_true",
