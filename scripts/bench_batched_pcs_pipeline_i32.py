...
**Batched** Zip+ with i32 evaluations.

This script runs the "Batched PCS Pipeline Suite i32 1row" criterion benchmarks
and collects the results into a LaTeX table.  The benchmarks batch 24
...
Usage:
    python3 scripts/bench_batched_pcs_pipeline_i32.py
    python3 scripts/bench_batched_pcs_pipeline_i32.py --no-run --input bench_output.txt
    python3 scripts/bench_batched_pcs_pipeline_i32.py --output batched_pcs_table_i32.tex
"""
...
BENCH_FILTER = "Batched PCS Pipeline Suite i32 1row"
CARGO_BENCH_CMD = [
    "cargo", "bench",
...
# Number of polynomials in the batch (must match BATCH_SIZE in benchmarks).
BATCH_SIZE = 24
...
def parse_criterion_output(
    ...
    """
    Parse criterion benchmark output for the batched PCS pipeline suite (i32).
...
def generate_latex_table(
    ...
    lines = [
        ...
        r"\label{tab:batched-pcs-pipeline-suite-i32-1row-r4}",
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Batched PCS pipeline (Encode/Merkle/Commit/Test/Verify) benchmarks for i32 and produce a LaTeX table."
    )
    parser.add_argument(
        "--no-run",
        action="store_true",
