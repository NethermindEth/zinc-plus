...
// ============================================================================
// Fused Multiply-Add: Combines BinaryPoly × twiddle + accumulator in one operation
// ============================================================================

/// Trait for fused multiply-add operations.
/// Computes: self += poly * scalar without intermediate allocation.
pub trait FusedMulAdd<Poly, Scalar> {
    /// Fused multiply-add: self += poly * scalar
    fn fused_mul_add(&mut self, poly: &Poly, scalar: Scalar);
}

/// NEON implementation of fused multiply-add for BinaryU64Poly<N> * i64 -> DensePolynomial<i64, N>
#[cfg(all(feature = "simd", target_arch = "aarch64"))]
impl<const N: usize> FusedMulAdd<BinaryU64Poly<N>, i64> for DensePolynomial<i64, N> {
    #[inline(always)]
    fn fused_mul_add(&mut self, poly: &BinaryU64Poly<N>, scalar: i64) {
        unsafe {
            fused_mul_add_neon::<N>(&mut self.coeffs, poly.inner(), scalar);
        }
    }
}
...
#[cfg(all(feature = "simd", target_arch = "aarch64"))]
#[inline(always)]
#[allow(unsafe_op_in_unsafe_fn, clippy::arithmetic_side_effects)]
unsafe fn fused_mul_add_neon<const N: usize>(acc: &mut [i64; N], mask_ref: &u64, scalar: i64) {
    ...
    while i + 8 <= N {
        ...
        let nz_signed: int8x8_t = vreinterpret_s8_u8(nz);
        let s16: int16x8_t = vmovl_s8(nz_signed);
        let lo32: int32x4_t = vmovl_s16(vget_low_s16(s16));
        let hi32: int32x4_t = vmovl_s16(vget_high_s16(s16));

        let m0: int64x2_t = vmovl_s32(vget_low_s32(lo32));
        ...
        // Load accumulator, add masked scalar, store back
        let acc_ptr = acc.as_mut_ptr().add(i);
        let a0 = vld1q_s64(acc_ptr);
        let a1 = vld1q_s64(acc_ptr.add(2));
        let a2 = vld1q_s64(acc_ptr.add(4));
        let a3 = vld1q_s64(acc_ptr.add(6));

        // Add (scalar & mask) to accumulator: scalar & -1 = scalar, scalar & 0 = 0
        vst1q_s64(acc_ptr, vaddq_s64(a0, vandq_s64(scalar_v, m0)));
        vst1q_s64(acc_ptr.add(2), vaddq_s64(a1, vandq_s64(scalar_v, m1)));
        vst1q_s64(acc_ptr.add(4), vaddq_s64(a2, vandq_s64(scalar_v, m2)));
        vst1q_s64(acc_ptr.add(6), vaddq_s64(a3, vandq_s64(scalar_v, m3)));
...
/// AVX-512 implementation of fused multiply-add
#[cfg(all(feature = "simd", target_arch = "x86_64", target_feature = "avx512f"))]
impl<const N: usize> FusedMulAdd<BinaryU64Poly<N>, i64> for DensePolynomial<i64, N> {
    #[inline(always)]
    fn fused_mul_add(&mut self, poly: &BinaryU64Poly<N>, scalar: i64) {
        unsafe {
            fused_mul_add_avx512::<N>(&mut self.coeffs, poly.inner(), scalar);
        }
    }
}
...
#[inline(always)]
#[allow(unsafe_op_in_unsafe_fn, clippy::arithmetic_side_effects)]
unsafe fn fused_mul_add_avx512<const N: usize>(acc: &mut [i64; N], mask_ref: &u64, scalar: i64) {
    ...
    while i + 8 <= N {
        ...
        // Load accumulator
        let acc_ptr = acc.as_mut_ptr().add(i) as *mut __m512i;
        let a = _mm512_loadu_si512(acc_ptr);

        // Create masked scalar (scalar where bit is set, 0 otherwise)
        let s = _mm512_maskz_set1_epi64(kmask, scalar);

        // Add and store
        _mm512_storeu_si512(acc_ptr, _mm512_add_epi64(a, s));

        i += 8;
    }

    // Tail: handle remaining coefficients
    while i < N {
        let bit = ((mask64 >> i) & 1) != 0;
        if bit {
            acc[i] += scalar;
...
/// Fallback (non-SIMD) implementation of fused multiply-add
#[cfg(not(any(
