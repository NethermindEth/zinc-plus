...
fn display<R: Display>(coeffs: &[R], f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    write!(f, "[")?;
    let mut first = true;
...
fn neg<R: Neg<Output = R>>(coeffs: Vec<R>) -> Vec<R> {
...
macro_rules! impl_assign_op {
    ($name:ident, $trait:ident) => {
        ...
        fn $name<R: for<'a> $trait<&'a R> + Clone>(
            lhs: &mut Vec<R>,
            rhs: &[R],
...
#[allow(clippy::arithmetic_side_effects)]
fn mul<R: Semiring, const CHECK: bool>(
    lhs: &[R],
    rhs: &[R],
