...
//!
//! Given J shifted-MLE claims `y_j = \tilde{v}^{down}_j(r)` for trace columns
//! `\tilde{v}_j` and the shared evaluation point `r`, the protocol proves:
//!
//! ```text
...
use zinc_utils::{inner_transparent_field::InnerTransparentField, sub};
...
pub struct BatchedShift<F>(PhantomData<F>);

impl<F> BatchedShift<F>
where
    ...
    F::Modulus: ConstTranscribable,
{
    ...
    pub fn prove_as_subprotocol(
        ...
        // Step 1: Sample batching coefficients \gamma_1,...,\gamma_J
        let gammas: Vec<F> = transcript.get_field_challenges(num_cols, field_cfg);

        // Step 2: Build shift_r MLE (\tilde{next}(r, ?))
        ...
        // comb_fn([sd, v_1, ..., v_J]) = sd * Σ_j(\gamma_j * v_j)
    ...
    pub fn verify_as_subprotocol(
        ...
        // Step 2: Compute expected sum = Σ_j \gamma_j * y_j
        ...
        // shift_r(ρ) = \tilde{next}(r, ρ)  (zero-padded convention, no clamping term)
...
/// Build the shift selector MLE `\tilde{next}(r, ?)` with the first `num_vars`
/// variables fixed to `r`.
///
/// For each `b \in {0,1}^{num_vars}`:
///   `shift_r(b) = \tilde{next}̃(r, b)`
///
/// Uses the identity `\tilde{next}(r, b) = \tilde{eq}(r, b−1)` for `b > 0` and
/// `0` for `b = 0`.
///
...
/// 0`.
///
/// Note: the paper uses a clamping variant
/// ```text
/// shiftdown_r(b) = \tilde{next}(r, b) + eq(r,1) * eq(b,1)
/// ```
/// which repeats the last row instead of zero-padding. We use the simpler
/// zero-padded version to match the existing code convention.
    ...
    F::Inner: Zero + Default,
{
    ...
    // \tilde{next}(r, 0) = 0
    // \tilde{next}(r, b) = eq(r, b-1) for b > 0
