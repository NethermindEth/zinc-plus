...
/// Sumcheck for products of multilinear polynomial, generic over the field
/// config `C`.
pub struct MLSumcheck<C>(PhantomData<C>);
...
/// the field config is bound into the transcript separately, at sampling
/// time / at the top level of the surrounding proof.
impl<F: ConstTranscribable> GenTranscribable for SumcheckProof<F> {
    fn read_transcription_bytes_exact(bytes: &[u8]) -> Self {
        ...
        for _ in 0..n_msgs {
            ...
            bytes = rest;
            let end = mul!(len, F::NUM_BYTES);
            let tail_evaluations: Vec<F> = Vec::read_transcription_bytes_exact(&bytes[..end]);
        ...
        let claimed_sum = F::read_transcription_bytes_exact(bytes);
        Self {
            messages,
            claimed_sum,
        }
    ...
    fn write_transcription_bytes_exact(&self, mut buf: &mut [u8]) {
        ...
        self.claimed_sum.write_transcription_bytes_exact(buf);
    }
}

impl<F: ConstTranscribable> Transcribable for SumcheckProof<F> {
    ...
    fn get_num_bytes(&self) -> usize {
        ...
        u32::NUM_BYTES // n_msgs
            ...
            + total_evals * F::NUM_BYTES // evals
            + F::NUM_BYTES // claimed_sum
    }
}
    ...
    C::Integer: ConstTranscribable,
{
    ...
    pub fn prove_as_subprotocol(
        ...
        config: &C,
    ) -> (SumcheckProof<C::Element>, ProverState<C>) {
        if nvars == 0 {
    ...
    pub fn verify_as_subprotocol(
        ...
        let (nvars_field, degree_field) = {
            (
                config.project(&(num_vars as u64)),
